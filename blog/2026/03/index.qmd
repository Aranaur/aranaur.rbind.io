---
title: "Сакура — 1 200 років весни в Кіото"
date: 2026-04-30
date-format: iso
description: "Візуалізація 1 200-річної історії цвітіння сакури в Кіото: від середньовічних хронік до сучасних метеорологічних даних. Як зміна клімату зсуває весну?"
image: "featured.jpg"
twitter-card:
  image: "featured.jpg"
open-graph:
...
citation:
  ...
  title: "Sakura — 1 200 years of spring in Kyoto"
  language: uk-UA
...
#| echo: false
#| output: false
# Скрипт sakura_viz.py запускається окремо через `uv run sakura_viz.py`
# і генерує featured.jpg та sakura_plot.png
import pandas as pd
import numpy as np
```
...
#| eval: false
#| code-fold: true
#| code-summary: "sakura_viz.py — повний код візуалізації"
...
Як взагалі можливо мати дані за 1 200 років? Усе завдяки кропіткій роботі японського кліматолога [Ясуюкі Аоно](https://scholar.google.com/citations?user=G9-fMrgAAAAJ&hl=ja). Він роками збирав по крихтах згадки про цвітіння з імператорських щоденників, хронік буддійських храмів, офіційних записів та навіть поетичних антологій.
...
```{python}
#| label: tbl-data-head
#| tbl-cap: "Так виглядають сирі дані: рік і день року (1 = 1 січня), коли сакура досягала повного розкриття (mankai)."
import pandas as pd
...
- <span style="color:#c9184a">**Насичений рожевий**</span> — сакура зацвіла значно раніше історичного середнього (тепла весна).
- <span style="color:#1a6a9a">**Глибокий синій**</span> — цвітіння запізнилося (холодна весна).
...
## 🛠️ Технічні деталі візуалізації
...
- **Датасет:** [Cherry Blossom Data on Kaggle](https://www.kaggle.com/) / [Our World in Data](https://ourworldindata.org/grapher/date-of-the-peak-cherry-tree-blossom-in-kyoto).
:::
