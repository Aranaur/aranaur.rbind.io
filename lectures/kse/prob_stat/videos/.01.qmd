...
tibble(value = c(binomial, normal),
       type = rep(c("Binomial", "Normal"), each = n)) %>% 
    ggplot(aes(x = value, fill = type)) +
    geom_histogram() +
    labs(title = "Normal approximation of Binomial",
         x = "Value",
         y = "Density",
         fill = "") +
    theme_minimal() +
    scale_fill_manual(values = c(turquoise, red), labels = c("Binomial", "Normal"))
```