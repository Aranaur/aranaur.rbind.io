...
> You have bought a beautiful new dice, and you want to check if it is true that all its faces fall out equally likely.

$X_1, \ldots, X_n$ — outcomes of $n$ rolls of the dice.

$\mathcal{P}$ --- true probabilities of the dice faces.

$\mathcal{P}_0$ --- hypothesized probabilities of the dice faces.
...
where $F, F_0$ &mdash; *distribution functions*.
...
1. We estimate the CDF, or $F(x)$:
    ...
    - Empirical CDF $\hat{F}(x) = \frac{1}{n} \sum_{i=1}^n I(X_i \leq x) \stackrel{p}{\rightarrow} F(x)$.
...
If we take a point $x$ such that $|F(x) - F_0(x)|$ is maximal (this is $\sup$), then this difference tends to zero.
...
Let's replace $F(x)$ with an estimate: the empirical distribution function. Then the **statistic of the test will be**
...
- $H_0: D_n \stackrel{p}{\rightarrow} 0$.
...
>
> Let $X$ be sampled from a continuous distribution. Then at $H_0$ $\sqrt{n}D_n \stackrel{d}{\rightarrow} \phi$, where $\phi$ has a **Kolmogorov distribution**: $F_\phi(x) = \begin{equation*} 
 \begin{cases}
...
- For $X$ the Shapiro-Wilk test = `{python} round(shapiro(X)[0], 4)` and $p$-value = `{python} round(shapiro(X)[1], 2)`. 

For $Y$: `{python} round(shapiro(Y)[0], 4)` and `{python} round(shapiro(Y)[1], 2)`.

For $X/Y$: `{python} round(shapiro(X/Y)[0], 4)` and `{python} round(shapiro(X/Y)[1], 2)`.
...
- Then, the score of the $i$-th quantile &mdash; $i$ value in the sorted sample (recall the empirical distribution function). $\widehat{F_n}(x) = \dfrac{1}{N}\sum([X_i \leq x])$.
...
## Example: Student's distribution
