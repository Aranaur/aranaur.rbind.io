...
# Last year, in this room {background-iframe="colored-particles/index.html"}
...
# What this course is {background-iframe="colored-particles/index.html"}
...
- **From Week 4, the practice session is a worked example and the graded work goes home**, due 23:59 the same day. You get your own series in Week 4, and from Week 6 a benchmark you have to beat on it.
:::
...
| Accuracy | Not graded until the project | **Graded from Week 6**, against your own benchmark |
...
- **The lab runs a week behind the lecture.** Week N's practice is built on week N−1's lecture, which is published and a week old by then — so missing a lecture never costs you the lab that follows it.
- **Week 1 (today)**: onboarding. Environment, data, submission mechanics. **Nothing is graded today.**
- **Weeks 2–3**: Track A — foundational labs.
- **Week 4, 6, 7, 8, 9**: Track B — forecasting challenges, built in the session and finished at home.
...
# The rules {background-iframe="colored-particles/index.html"}
...
## Track B — the challenges (Weeks 4, 6, 7, 8, 9) {.smaller}
...
- Five challenges on the same series: the forecaster's toolbox → ETS → ARIMA → seasonal ARIMA → advanced methods.
...
**Week 4**, the baseline lab on the same series — accuracy is measured and published, not marked:

| Criteria | Points |
|---|---:|
| Decomposition and forecast | 3 |
| Justification block | 3 |
| EDA and visualisation | 1 |
| Code quality and reproducibility | 1 |
...
- The pool is **pre-screened**, and the screen is what makes the accuracy points fair: a series is issued only if it is seasonal, if an automatic ETS **and** an automatic ARIMA each beat its benchmark on their own, and if the better of the two lands between **0.70 and 0.90** of it. Below that band the series is trivial; above it, effectively unbeatable.
:::

::: {.fragment}
[Every series you can be assigned is one that the week's own method is known to beat.]{.hi}

If you do not beat it, that is a fact about your model, not about your luck. Week 4 is the exception,
and that is exactly why it is not marked on accuracy: with decomposition alone the benchmark falls
about a third of the time, which measures the draw rather than the work.
:::

## One deadline: 23:59 {.smaller}

An 80-minute slot is not enough to specify a model by hand, justify it, and package a forecast — and you asked not to have to. So the two are split:

::: {.incremental}
1. **The practice session** is a worked example on a shared series: the whole recipe, out loud, with every decision argued. Nothing is submitted, nothing is marked, attendance is not taken.
2. **23:59 the same day** — your submission, from home, on your own series.
:::
...
| EDA, methodology and diagnostics — residual analysis is required | 6 |
...
# The fine print {background-iframe="colored-particles/index.html"}
...
::: {.fragment}
The session assumes you arrive already familiar with the week's method — the lecture it builds on was published a week earlier. **80 minutes is enough to see a method applied end to end, and nowhere near enough to learn one from scratch.**
:::
...
In the final project you will take a **foundation model** — a network trained on millions of time series, which has never seen your data — and run it with **zero training** on the dataset everyone is forecasting.

Then you will compare it against the model you built by hand over nine weeks.
