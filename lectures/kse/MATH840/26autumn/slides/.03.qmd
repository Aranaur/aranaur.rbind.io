...
subtitle: "MATH840 | Time Series | Week 3"
...
#| echo: true
#| code-fold: false
url = '../data/aus_production_long.csv'
...
plot_series(df, fcasts_naive,
            xlabel="Quarter",
            ylabel="Production",
            title="Naïve method forecasts for Bricks production")
```

## Seasonal naïve method {.smaller}
...
plot_series(df, fcasts_snaive,
            xlabel="Quarter",
            ylabel="Production",
            title="Seasonal naïve method forecasts for Bricks")
```
...
## Example: Australian beer production {.tiny}
...
## Example: Google’s daily closing stock price {.tiny}

```{python}
url = '../data/gafa_stock.csv'
...
# 1. Створення фігури та двох підграфіків
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 7))

# --- Встановлюємо білий фон для фігури та осей ---
...
# --- Графік 1: Асиметричний (вихідний) розподіл ---

# Генеруємо дані для правостороннього асиметричного розподілу
a = 4  # Параметр асиметрії
x_skewed = np.linspace(stats.skewnorm.ppf(0.001, a), stats.skewnorm.ppf(0.999, a), 200)
pdf_skewed = stats.skewnorm.pdf(x_skewed, a)

# Розраховуємо моду, медіану та середнє
mean_val = stats.skewnorm.mean(a)
median_val = stats.skewnorm.median(a)
mode_val = x_skewed[np.argmax(pdf_skewed)] # Мода - це пік розподілу

# Малюємо криву розподілу
ax1.plot(x_skewed, pdf_skewed, 'k', linewidth=1.5)

# Малюємо вертикальні лінії для моди, медіани та середнього
...
# --- Графік 2: Симетричний (трансформований) розподіл ---

# Генеруємо дані для нормального розподілу
...
# Малюємо центральну лінію (середнє = медіана = мода)
ax2.vlines(0, 0, stats.norm.pdf(0), 'k', lw=2)
ax2.text(0, stats.norm.pdf(0) + 0.05, 'symmetric', ha='center', fontsize=16)

# --- Стрілки та анотації ---

# 1. Головна чорна стрілка: з'єднує середнє симетричного розподілу з медіаною асиметричного
con = ConnectionPatch(
  xyA=(0, 0), xyB=(median_val, 0),
  coordsA='data', coordsB='data',
  axesA=ax2, axesB=ax1,
  arrowstyle="->", shrinkB=5,
  connectionstyle="arc3,rad=-0.4", # Вигин стрілки вниз
  color='k', lw=2
)
ax2.add_artist(con)

# 2. Зелена анотація з поясненням
ax1.annotate(
  'adjust to get to\nthe mean',
...
# --- Фінальне налаштування вигляду графіків ---
for ax in [ax1, ax2]:
  ax.set_yticks([])
  ax.set_xticks([])
  # Малюємо осі X та Y
  ...
  ax.set_ylim(bottom=-0.3) # Додаємо місце знизу для анотацій
...
- Forecast $\hat{S_t}$ using [seasonal naïve method]{.hi}
...
#| fig-align: center
#| code-fold: false
url = '../data/us_employment_titles.csv'
...
sf = StatsForecast(models=[seasonal_naive_method], freq="ME")
...
# Встановлюємо межі, щоб було достатньо місця
ax.set_ylim(-1, 1)
ax.set_xlim(-1, 26)

# Прибираємо осі та рамку для чистого вигляду
ax.axis('off')

# --- Визначення даних та кольорів ---
# Кількість точок для тренувальних та тестових даних
n_train = 18
n_test = 5

# Створюємо масиви з позиціями точок на осі X
train_points = np.arange(0, n_train)
test_points = np.arange(n_train, n_train + n_test)

# Кольори, що відповідають зображенню
train_color = '#377eb8'  # Синій
test_color = '#d95f02'   # Помаранчевий
axis_color = 'black'

# --- Малюємо елементи графіку ---

# 1. Головна вісь часу зі стрілкою
# Малюємо лінію, а потім додаємо стрілку за допомогою annotate
ax.axhline(y=0, color=axis_color, zorder=1)
# Використовуємо annotate для створення стрілки на кінці
ax.annotate('', xy=(n_train + n_test + 1, 0), xytext=(-1, 0),
            arrowprops=dict(arrowstyle="->", color=axis_color))


# 2. Малюємо точки даних
# Тренувальні дані
...
# 3. Додаємо текстові підписи
# Підпис для тренувальних даних
...
# Визначаємо функцію квантильного рахунку (pinball loss)
def quantile_score(error, p):
    """
    Розраховує квантильний рахунок.
    error: y_true - y_pred
    p: квантиль (0 < p < 1)
    """
...
# Функція для оновлення кожного кадру анімації
def update(frame):
    p = p_values[frame]
    qs = quantile_score(error_range, p)
    
    line.set_data(error_range, qs)
    title.set_text(f"Залежність $Q_{{p,t}}$ від помилки при $p = {p:.2f}$")
    
    # Оновлюємо колір лінії для кращої візуалізації
    # Колір змінюється від синього (p~1) до червоного (p~0)
...
ani = animation.FuncAnimation(
    ...
    init_func=init, 
    blit=True,
    interval=200  # Інтервал між кадрами в мілісекундах
)
...
# Зберігаємо анімацію як GIF
gif_path = os.path.join(output_dir, 'quantile_score_animation.gif')
ani.save(gif_path, writer='pillow', fps=5)

# Для відображення в Jupyter/Quarto
plt.close(fig) # Запобігаємо статичному відображенню
```
...
# statsforecast >= 2 requires every level strictly inside (0, 100): the 0% interval has
# zero width and adds nothing to the CRPS approximation, so the grid starts at 0.1.
levels = list(np.round(np.arange(1, 1000) / 10, 1))
...
## Scale-free comparisons using skill scores {.tiny}
...
def plot_ts_cross_validation(
    ...
    h=1,
    step=1,
    window_size=25,  # Новий параметр для фіксованої кількості точок
    ax=None
):
    """
    Візуалізує перехресну валідацію для часових рядів з фіксованим вікном.

    Параметри:
    - n_folds (int): Кількість ітерацій (зрізів) валідації.
    - initial_train_size (int): Початковий розмір тренувального набору.
    - h (int): Горизонт прогнозування.
    - step (int): Крок, на який збільшується тренувальний набір.
    - window_size (int): Загальна кількість точок для відображення в кожному рядку.
    - ax (matplotlib.axes.Axes, optional): Вісь для малювання.
    """
    # Кольори
    train_color = blue
    test_color = orange
    ...
    # Створення графіку, якщо не надано існуючий
    if ax is None:
        fig, ax = plt.subplots(figsize=(12, 8))
    
    # Ітерація по кожному зрізу перехресної валідації
    for i in range(n_folds):
        y_pos = n_folds - 1 - i  # Позиція по осі Y (малюємо згори донизу)
        
        # Розмір тренувального набору для поточного зрізу
        current_train_size = initial_train_size + i * step
        
        # Визначаємо точки для прогнозу
        forecast_start = current_train_size
        forecast_end = forecast_start + h
        
        # Визначаємо точки для візуалізації в межах window_size
        all_points = np.arange(window_size)
        
        # Розподіляємо точки по категоріях
        ...
        # 1. Малюємо вісь часу зі стрілкою
...
# Приклад використання функції для відтворення оригінального малюнка
