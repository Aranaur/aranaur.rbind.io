---
title: "Regression Analysis, Part 2"
subtitle: "Descriptive Statistics"
author: "Ihor Miroshnychenko"
institute: Kyiv School of Economics
from: markdown+emoji
title-slide-attributes:
    data-background-iframe: .10_files/libs/colored-particles/index.html
...
library(tidyverse)
library(pacman)
p_load(broom, latex2exp, ggplot2, ggthemes, ggforce, viridis, dplyr, magrittr, knitr, parallel, future, future.apply)
...
# Multiple regression {background-iframe=".10_files/libs/colored-particles/index.html"}
...
```{R, mult reg plot 3, dev = "svg", echo = F}
#| fig-height: 10
...
```{R, mult reg plot 4, dev = "svg", echo = F}
#| fig-height: 10
ggplot(data = gen_df %>% mutate(y = y - 4 * x2), aes(y = y_dm, x = x1)) +
...
```{R, mult reg plot 5, dev = "svg", echo = F}
#| fig-height: 10
ggplot(data = gen_df, aes(y = y, x = x1, color = x2, shape = x2)) +
...
[
- Generate 10,000 observations on $y$<br>
- Generate 10,000 observations on variables $x_1$ through $x_{1000}$<br>
- Regressions<br>
  &emsp;&emsp;- LM<sub>1</sub>: Regress $y$ on $x_1$; record R<sup>2</sup><br>
  &emsp;&emsp;- LM<sub>2</sub>: Regress $y$ on $x_1$ and $x_2$; record R<sup>2</sup><br>
  &emsp;&emsp;- LM<sub>3</sub>: Regress $y$ on $x_1$, $x_2$, and $x_3$; record R<sup>2</sup><br>
  &emsp;&emsp;- ...<br>
  &emsp;&emsp;- LM<sub>1000</sub>: Regress $y$ on $x_1$, $x_2$, ..., $x_{1000}$; record R<sup>2</sup>
]{.pseudocode-small}
...
set.seed(1234)
y <- rnorm(size1)
x <- matrix(data = rnorm(size2), nrow = size1)
x <- cbind(matrix(data = 1, nrow = size1, ncol = 1), x)

r_df <- future_lapply(1:(1000-1), FUN = function(i) {
...
**One solution:** [Adjusted]{.pink} $\color{#e64173}{R^2}$

```{R, adjusted r2 plot, echo = F, dev = "svg"}
#| fig-height: 10
ggplot(data = r_df, aes(x = k, y = r2)) +
geom_hline(yintercept = 0) +
geom_vline(xintercept = 0) +
...
# Omitted-variable bias {background-iframe=".10_files/libs/colored-particles/index.html"}
...
# Interpreting coefficients {background-iframe=".10_files/libs/colored-particles/index.html"}
...
annotate(
  ...
  label = TeX("$\\hat{\\beta}_0 = \\bar{{Group}_0}$"),
  size = 7
) +
annotate(
  geom = "text",
  ...
  label = TeX("$\\hat{\\beta}_0 + \\hat{\\beta}_1 = \\bar{{Group}_1}$"),
  size = 7,
  color = red_pink
) +
theme_empty
...
You will frequently see logged outcome variables with linear (non-logged) explanatory variables, _e.g._,
...
Similarly, frequently employ log-log models, in which the outcome variable is logged *and* at least one explanatory variable is logged
...
## Log-linear with a binary variable {.smaller}
...
# Additional topics {background-iframe=".10_files/libs/colored-particles/index.html"}
...
$$
y_i = \color{#e64173}{\hat{\beta}_{0}} + \color{#e64173}{\hat{\beta_1}} \, x_{1i} + \color{#e64173}{\hat{\beta_2}} \, x_{2i} + \cdots + \color{#e64173}{\hat{\beta}_{k}} \, x_{ki} + e_i
$$
...
- **Prediction:** correlation, $\hat{y}_i$ (low error), model selection, nonlinear models are much more common

## Treatment effects and causality {.smaller}
...
- [Recreational-cannabis legalization](https://onlinelibrary.wiley.com/doi/abs/10.1111/ecin.12751)
...
- Replace outliers with the 99 percentile of their variable (*Windsorize*)
...
If you run a regression with missing values, [R]{.mono} drops the observations missing those values.

If the observations are missing in a nonrandom way, a random sample may end up nonrandom.

# Questions? {.unnumbered .unlisted background-iframe=".10_files/libs/colored-particles/index.html"}
