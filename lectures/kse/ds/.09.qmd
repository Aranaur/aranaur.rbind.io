---
title: "Regression Analysis, Part 1"
...
# Population *vs.* sample {background-iframe=".09_files/libs/colored-particles/index.html"}
...
- Differences between individual samples and the population lead to **uncertainty**.
:::
...
- Correcting for heteroskedasticity and autocorrelation[^auto]

. . .

First, let's refresh on how we get these (uncertain) regression estimates.

[^auto]: Not covered in this course.
...
**Recall:** Minimizing a multivariate function requires (**1**) first derivatives equal zero (the *1[st]{.super}-order conditions*) and (**2**) second-order conditions (concavity).
...
In statistics, we generally stick with unbiased (or consistent) estimators. But other disciplines (especially computer science) think a bit more about this tradeoff.
...
There are many ways how real life may **violate these assumptions**. For instance:
...
scale_y_continuous(breaks = lm0$coefficients[2], labels = ("β1")) +
...
`R` default to testing hypotheses against the value [zero]{.hi}.
...
You will sometimes see $F$ tests.
