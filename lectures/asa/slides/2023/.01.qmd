...
# Генеральна сукупність і вибірка {background-color="#eef3f8"}
...
Ці PDF-и характеризують розподіли ймовірностей, а найпоширеніші/відомі/популярні розподіли мають назви (_наприклад_, нормальний, *t*, Гамма).
...
Імовірність того, що стандартна нормальна випадкова змінна набуває значення від -1,96 до 1,96: $\mathop{\text{P}}\left(-1,96 \leq X \leq 1,96\right) = 0,95$
...
Наприклад, 100 спостережень, два коефіцієнти (_тобто_, $\hat{\beta}_0$ і $\hat{\beta}_1 \implies k = 2$), і $\alpha = 0,05$ ( для 95% довірчого інтервалу) дає нам $t_{0,025,\,98} = `r round(qt(0.025, 98), 2)`$
...
$t_\text{stat} = 10.811$ та $t_\text{0.975, 98} = `r round(qt(0.975, 98), 2)`$
...
Для кожного з 10 000 зразків ми обчислимо статистику $t$, а потім побачимо, скільки статистичних даних $t$ перевищує наше критичне значення (`r round(qt(0.975, 28), 2)`, як вище).
