---
title: "Гетероскедастичність"
subtitle: "Прикладний статистичний аналіз"
author: "Ігор Мірошниченко"
institute: КНУ імені Тараса Шевченка | ФІТ
# date: today
# date-format: iso
from: markdown+emoji
title-slide-attributes:
  data-background-iframe: .04_files/libs/colored-particles/index.html
#   data-background-color: "#eef3f8"
language: _language-ua.yml
footer: 🔗 <a href="https://aranaur.rbind.io/lectures/asa/">Матеріали курсу</a>
format: 
  revealjs:
    transition: fade
    chalkboard: true
    logo: img/fit.png
    code-line-numbers: false
    slide-number: c/t
    mouse-wheel: true
    width: 1350  
    height: 759.375
    highlight-style: github
    pdf-separate-fragments: true
    # fig-height: 7.5
    # fig-width: 15
    fig-align: center
    fig-format: svg
    theme: [default, custom.scss]
    toc: true
    toc-depth: 1
    fig-width: 15
# jupyter: python3
execute: 
  warning: false
  echo: true
  # cache: true

---

```{python}
#| label: setup-python
#| include: false

# Import libraries
import numpy as np
import pandas as pd
import hashlib
import matplotlib.pyplot as plt
import seaborn as sns

from IPython.display import Markdown
from tabulate import tabulate

# Set seaborn default style
sns.set_style('whitegrid')
sns.set_palette('colorblind')

# Define colors
red_pink   = "#e64173"
turquoise  = "#20B2AA"
orange     = "#FFA500"
red        = "#fb6107"
blue       = "#181485"
navy       = "#150E37FF"
green      = "#8bb174"
yellow     = "#D8BD44"
# grey_light = "grey70"
# grey_mid   = "grey50"
# grey_dark  = "grey20"
purple     = "#6A5ACD"
slate      = "#314f4f"
```

```{r}
#| label: setup
#| include: false

library(pacman)
p_load(broom, latex2exp, ggplot2, ggthemes, viridis, extrafont,  dplyr,  magrittr, knitr, parallel, fontawesome, kableExtra)

# Define pink color
red_pink   = "#e64173"
turquoise  = "#20B2AA"
orange     = "#FFA500"
red        = "#fb6107"
blue       = "#3b3b9a"
green      = "#8bb174"
grey_light = "grey70"
grey_mid   = "grey50"
grey_dark  = "grey20"
purple     = "#6A5ACD"
slate      = "#23373b"


# A blank theme for ggplot
theme_empty = theme_bw() + theme(
  line = element_blank(),
  rect = element_blank(),
  strip.text = element_blank(),
  axis.text = element_blank(),
  plot.title = element_blank(),
  axis.title = element_blank(),
  plot.margin = structure(c(0, 0, -0.5, -1), unit = "lines", valid.unit = 3L, class = "unit"),
  legend.position = "none"
)
theme_simple = theme_bw() + theme(
  line = element_blank(),
  panel.grid = element_blank(),
  rect = element_blank(),
  strip.text = element_blank(),
  axis.text.x = element_text(size = 18, family = "STIXGeneral"),
  axis.text.y = element_blank(),
  axis.ticks = element_blank(),
  plot.title = element_blank(),
  axis.title = element_blank(),
  # plot.margin = structure(c(0, 0, -1, -1), unit = "lines", valid.unit = 3L, class = "unit"),
  legend.position = "none"
)
theme_axes_math = theme_void() + theme(
  text = element_text(family = "MathJax_Math"),
  axis.title = element_text(size = 22),
  axis.title.x = element_text(hjust = .95, margin = margin(0.15, 0, 0, 0, unit = "lines")),
  axis.title.y = element_text(vjust = .95, margin = margin(0, 0.15, 0, 0, unit = "lines")),
  axis.line = element_line(
    color = "grey70",
    size = 0.25,
    arrow = arrow(angle = 30, length = unit(0.15, "inches")
  )),
  plot.margin = structure(c(1, 0, 1, 0), unit = "lines", valid.unit = 3L, class = "unit"),
  legend.position = "none"
)
theme_axes_serif = theme_void() + theme(
  text = element_text(family = "MathJax_Main"),
  axis.title = element_text(size = 22),
  axis.title.x = element_text(hjust = .95, margin = margin(0.15, 0, 0, 0, unit = "lines")),
  axis.title.y = element_text(vjust = .95, margin = margin(0, 0.15, 0, 0, unit = "lines")),
  axis.line = element_line(
    color = "grey70",
    size = 0.25,
    arrow = arrow(angle = 30, length = unit(0.15, "inches")
  )),
  plot.margin = structure(c(1, 0, 1, 0), unit = "lines", valid.unit = 3L, class = "unit"),
  legend.position = "none"
)
theme_axes = theme_void() + theme(
  text = element_text(family = "Fira Sans Book"),
  axis.title = element_text(size = 18),
  axis.title.x = element_text(hjust = .95, margin = margin(0.15, 0, 0, 0, unit = "lines")),
  axis.title.y = element_text(vjust = .95, margin = margin(0, 0.15, 0, 0, unit = "lines")),
  axis.line = element_line(
    color = grey_light,
    size = 0.25,
    arrow = arrow(angle = 30, length = unit(0.15, "inches")
  )),
  plot.margin = structure(c(1, 0, 1, 0), unit = "lines", valid.unit = 3L, class = "unit"),
  legend.position = "none"
)
```

#  {.unlisted .unnumbered}

::: {.callout-important}
## Правила поведінки при сигналі "Повітряна тривога" під час дистанційного навчання

1. При отриманні сигналу **"Повітряна тривога" в місті Києві** лекційне заняття **припиняється**, а студенти мають перейти **до укриттів** за місцем перебування. Після отримання сигналу **"Відбій повітряної тривоги"** лекційне заняття буде **продовжено**.
2. При отриманні сигнали **"Повітряна тривога" в інших регіонах України**, студенти мають **від'єднатися** від лекційного заняття та перейти **до укриттів** за місцем перебування. Після отримання сигналу **"Відбій повітряної тривоги"** студенти можуть **повернутися** до лекційного заняття.
:::



# Основні положення

## Гетероскедастичність {.smaller}

Давайте згадаємо **поточні МНК припущення**

. . .

1. Наша вибірка ($x_k$ та $y_i$) була [випадковим чином]{.hi} відібрана з сукупності.

. . .

2. $y$ є [лінійною функцією]{.hi} $\beta_k$ і $u_i$.

. . .

3. У нашій вибірці немає ідеальної [мультиколінеарності]{.hi}.

. . .

4. Незалежні змінні є [екзогенними]{.hi}: $\mathop{\boldsymbol{E}}\left[ u \middle| X \right] = 0 \left(\implies \mathop{\boldsymbol{E}}\left[ u \right] = 0\right)$.

. . .

5. Збурення мають [постійну дисперсію]{.hi} $\sigma^2$ і [нульову коваріацію]{.hi}, _тобто_,
   - $\mathop{\boldsymbol{E}}\left[ u_i^2 \middle| X \right] = \mathop{\text{Var}} \left( u_i \middle| X \right) = \sigma^2 \implies \mathop{\text{Var}} \left( u_i \right) = \sigma^2$
   - $\mathop{\text{Cov}} \left( u_i, \, u_j \middle| X \right) = \mathop{\boldsymbol{E}}\left[ u_i u_j \middle| X \right] = 0$ для $i\neq j$

. . .

6. Збурення походять від [нормального]{.hi} розподілу, _тобто_, $u_i \overset{\text{iid}}{\sim} \mathop{\text{N}}\left( 0, \sigma^2 \right)$.

---

## Гетероскедастичність {.smaller}

Сьогодні ми зосереджуємось на припущенні \#5:

> 5. Збурення мають [постійну дисперсію]{.hi} $\sigma^2$ і [нульову коваріацію]{.hi}, _тобто_,
   - $\mathop{\boldsymbol{E}}\left[ u_i^2 \middle| X \right] = \mathop{\text{Var}} \left( u_i \middle| X \right) = \sigma^2 \implies \mathop{\text{Var}} \left( u_i \right) = \sigma^2$
   - $\mathop{\text{Cov}} \left( u_i, \, u_j \middle| X \right) = \mathop{\boldsymbol{E}}\left[ u_i u_j \middle| X \right] = 0$ для $i\neq j$

. . .

Зокрема, ми зосередимося на припущенні [постійної дисперсії]{.hi} (також відомому як *гомоскедастичність*).

. . .

**Порушення цього припущення:**

[Гетероскедастичність:]{.hi} $\mathop{\text{Var}} \left( u_i \right) = \sigma^2_i$ і $\sigma^2_i \neq \sigma^2_j$ для деяких $i\neq j$ .

. . .

Іншими словами: наші збурення мають різну дисперсію.

---

## Гетероскедастичність

Класичний приклад гетероскедастичності: воронка

Дисперсія $u$ зростає разом із $x$

```{r}
#| label: het-ex1
#| echo: false
#| fig-align: center

set.seed(12345)
ggplot(data = tibble(
  x = runif(1e3, -3, 3),
  e = rnorm(1e3, 0, sd = 4 + 1.5 * x)
), aes(x = x, y = e)) +
geom_point(color = "darkslategrey", size = 2.75, alpha = 0.5) +
labs(x = "x", y = "u") +
theme_axes_math
```

---

## Гетероскедастичність

Інший приклад гетероскедастичності: (подвійна воронка?)

Дисперсія $u$ зростає на крайніх значеннях $x$

```{r, het ex2 , dev = "svg", echo = F, fig.height = 5.5}
set.seed(12345)
ggplot(data = tibble(
  x = runif(1e3, -3, 3),
  e = rnorm(1e3, 0, sd = 2 + x^2)
), aes(x = x, y = e)) +
geom_point(color = "darkslategrey", size = 2.75, alpha = 0.5) +
labs(x = "x", y = "u") +
theme_axes_math
```

---

## Гетероскедастичність

Інший приклад гетероскедастичності

Різні групові дисперсії $u$ 

```{r, het ex3 , dev = "svg", echo = F, fig.height = 5.5}
set.seed(12345)
ggplot(data = tibble(
  g = sample(c(F,T), 1e3, replace = T),
  x = runif(1e3, -3, 3),
  e = rnorm(1e3, 0, sd = 0.5 + 2 * g)
), aes(x = x, y = e, color = g, shape = g, alpha = g)) +
geom_point(size = 2.75) +
scale_color_manual(values = c("darkslategrey", red_pink)) +
scale_shape_manual(values = c(16, 1)) +
scale_alpha_manual(values = c(0.5, 0.8)) +
labs(x = "x", y = "u") +
theme_axes_math
```

---

## Гетероскедастичність

[Гетероскедастичність]{.hi} присутня, коли дисперсія $u$ змінюється з будь-якою комбінацією наших пояснювальних змінних $x_1$ до $x_k$ (далі: $X$).

. . .

Це дуже поширене явище на практиці.

. . .

[Чому вона нам важлива:]{.hi} Гетероскедастичність показує нам, як незначні порушення наших припущень можуть вплинути на ефективність МНК.

---

## Наслідки

Отже, які наслідки гетероскедастичності? Зміщення? Неефективність?

По-перше, давайте перевіримо, чи це має наслідки для незміщенності МНК

. . .

**Нагадування~1~:** незміщенність МНК означає $\mathop{\boldsymbol{E}}\left[ \hat{\beta}_k \middle| X \right] = \beta_k$ для всіх $k$.

. . .

**Нагадування~2~:** раніше ми показували $\hat{\beta}_1 = \dfrac{\sum_i\left(y_i-\overline{y}\right)\left(x_i-\overline{x}\right)}{\sum_i\left(x_i -\overline{x}\right)^2}$

. . .

Це насправді допоможе нам переписати цю оцінку як

$$ \hat{\beta}_1 = \beta_1 + \dfrac{\sum_i \left( x_i - \overline{x} \right) u_i}{\sum_i \left( x_i - \overline{x} \right)^2} $$

---

## Наслідки {.smaller}

**Доведення:** Припускаємо, що $y_i = \beta_0 + \beta_1 x_i + u_i$

$$
\begin{aligned}
  \hat{\beta}_1
  &= \dfrac{\sum_i\left(y_i-\overline{y}\right)\left(x_i-\overline{x}\right)}{\sum_i\left(x_i -\overline{x}\right)^2} \\
  &= \dfrac{\sum_i\left(\left[ \beta_0 + \beta_1 x_i + u_i \right]- \left[ \beta_0 + \beta_1 \overline{x} + \overline{u} \right] \right)\left(x_i-\overline{x}\right)}{\sum_i\left(x_i -\overline{x}\right)^2} \\
  &= \dfrac{\sum_i\left(\beta_1 \left[ x_i - \overline{x} \right] + \left[u_i - \overline{u}\right]  \right)\left(x_i-\overline{x}\right)}{\sum_i\left(x_i -\overline{x}\right)^2} \\
  &= \dfrac{\sum_i\left(\beta_1 \left[ x_i - \overline{x} \right]^2 + \left[ x_i - \overline{x} \right] \left[u_i - \overline{u}\right]\right)}{\sum_i\left(x_i -\overline{x}\right)^2} \\
  &= \beta_1 + \dfrac{\sum_i\left(x_i - \overline{x}\right) \left(u_i - \overline{u}\right)}{\sum_i\left(x_i -\overline{x}\right)^2}
\end{aligned}
$$

---

## Наслідки {.smaller}

$$
\begin{aligned}
  \hat{\beta}_1
  &= \cdots = \beta_1 + \dfrac{\sum_i\left(x_i - \overline{x}\right) \left(u_i - \overline{u}\right)}{\sum_i\left(x_i -\overline{x}\right)^2} \\
  &= \beta_1 + \dfrac{\sum_i\left(x_i - \overline{x}\right) u_i - \overline{u} \sum_i\left(x_i - \overline{x}\right)}{\sum_i\left(x_i -\overline{x}\right)^2} \\
  &= \beta_1 + \dfrac{\sum_i\left(x_i - \overline{x}\right) u_i - \overline{u} \left(\sum_i x_i - \sum_i \overline{x}\right)}{\sum_i\left(x_i -\overline{x}\right)^2} \\
  &= \beta_1 + \dfrac{\sum_i\left(x_i - \overline{x}\right) u_i - \overline{u} \left(\sum_i x_i - n \overline{x}\right)}{\sum_i\left(x_i -\overline{x}\right)^2} \\
  &= \beta_1 + \dfrac{\sum_i\left(x_i - \overline{x}\right) u_i - \overline{u} \color{#e64173}{\left(\sum_i x_i - \sum_i x_i\right)}}{\sum_i\left(x_i -\overline{x}\right)^2} \\
  &= \beta_1 + \dfrac{\sum_i\left(x_i - \overline{x}\right) u_i}{\sum_i\left(x_i -\overline{x}\right)^2} \quad \text{😅}
\end{aligned}
$$

---

## Наслідки: зміщення {.smaller}

Тепер ми хочемо побачити, чи гетероскедастичність зміщує оцінку МНК для $\beta_1$.

. . .

$$
\begin{aligned}
  \mathop{\boldsymbol{E}}\left[ \hat{\beta}_1 \middle| X \right]
  &= \mathop{\boldsymbol{E}}\left[ \beta_1 + \dfrac{\sum_i\left(x_i - \overline{x}\right) u_i}{\sum_i\left(x_i -\overline{x}\right)^2} \middle| X \right] \\[0.5em]
  &= \beta_1 + \mathop{\boldsymbol{E}}\left[ \dfrac{\sum_i\left(x_i - \overline{x}\right) u_i}{\sum_i\left(x_i -\overline{x}\right)^2} \middle| X \right] \\[0.5em]
  &= \beta_1 + \dfrac{\sum_i\left(x_i - \overline{x}\right)}{\sum_i\left(x_i -\overline{x}\right)^2} \color{#e64173}{\underbrace{\mathop{\boldsymbol{E}}\left[ u_i \middle| X \right]}_{=0}} \\[0.5em]
  &= \beta_1
\end{aligned}
$$

. . .

Фух... [МНК все ще незміщенні]{.hi} для $\beta_k$.

---

## Наслідки: ефективність

**Ефективність** і **інтерпретованість** МНК не витримують гетероскедастичності.

- За наявності гетероскедастичності МНК [не є більше найефективнішим]{.hi} (найкращий) лінійним незміщеним оцінювачем.

. . .

- Було б більш інформативним (ефективнішим) [зважити спостереження]{.hi} обернено до їхньої дисперсії $u_i$.

   - Низька вага спостереженням з високою дисперсією $u_i$.

   - Висока вага спостереженням з низькою дисперсією $u_i$.

   - Тепер у вас є ідея зваженого методу найменших квадратів (ЗМНК, WLS)

---

## Наслідки: інтерпретованість

За наявності гетероскедастичності [стандартні похибки МНК є зміщенними]{.hi} .

- Неправильні довірчі інтервали

- Проблеми для перевірки гіпотез (як $t$, так і $F$ тести)

---

## Рішення

1. **Тести** для визначення наявності гетероскедастичності.

2. **Засоби захисту** для (1) ефективності та (2) інтерпретованісті

---

## Тестування гетероскедастичності

Хоча у нас *можливо* є рішення для гетероскедастичності, ефективність наших оцінювачів залежить від наявності гетероскедастичності.

1. **Тест Гольдфельда-Квандта**

2. **Тест Уайта**

3. **Тест Бройша — Пагана**

. . .

Кожен із цих тестів зосереджується на тому факті, що ми можемо [використовувати залишки МНК]{.hi} $\color{#e64173}{e_i}$ [для оцінки генеральної сукупності]{.hi} $\color{#e64173}{u_i}$.

# Тест Гольдфельда-Квандта

## Тест Гольдфельда-Квандта

Зосереджується на конкретному типі гетероскедастичності: чи відрізняється дисперсія $u_i$ [між двома групами]{.hi}.

Пам'ятаєте, як ми використовували наші залишки для оцінки $\sigma^2$?

$$ s^2 = \dfrac{\text{SSE}}{n-1} = \dfrac{\sum_i e_i^2}{n-1} $$

Ми використаємо цю саму ідею, щоб визначити, чи є докази того, що наші дві групи відрізняються дисперсіями своїх збурень, фактично порівнюючи $s^2_1$ і $s^2_2$ від наших двох груп.

---

## Тест Гольдфельда-Квандта

### Алгоритм

1. Впорядкування спостережень за $x$

2. Розділіть дані на дві групи розміром n^⭑^
   - G~1~: перша третина
   - G~2~: остання третина

3. Побудуйте окремі регресії $y$ від $x$ для G~1~ і G~2~

4. Розрахуйте SSE~1~ і SSE~2~

5. Обчисліть статистику тесту G-Q

---

## Статистика Гольдфельда-Квандта {.smaller}

$$F_{\left(n^{\star}-k,\, n^{\star}-k\right)} = \dfrac{\text{SSE}_2/(n^\star-k)}{\text{SSE}_1/(n^\star-k)} = \dfrac{\text{SSE}_2}{\text{SSE}_1}$$

відповідає $F$ розподілу (за нульовою гіпотезою) із ступенями свободи $n^{\star}-k$ і $n^{\star}-k$[^1].

::: {.callout-note}
- Тест G-Q вимагає, щоб збурення відповідали нормальним розподілам.
- G-Q передбачає дуже специфічний тип/форму гетероскедастичності.
- Дуже добре працює, якщо ми знаємо форму потенційної гетероскедастичності.
:::

[^1]: Голдфельд і Квандт запропонували розрахунок $n^{\star}$ з виразу $(3/8)n$. Значення $k$ --- це кількість оцінених параметрів (_тобто_, $\hat{\beta}_j$).

---

## Візуалізація тесту Гольдфельда-Квандта

```{r, gq1a, echo = F, dev = "svg", fig.height = 4}
set.seed(12345)
# Data
gq_df = tibble(
  x = runif(1e3, -3, 3),
  e = rnorm(1e3, 0, sd = 4 + 1.5 * x),
  y = 1 + 3 * x + e
)
# Quantiles
gq_x = quantile(gq_df$x, probs = c(3/8, 5/8))
# Regressions
sse1 = lm(y ~ x, data = gq_df %>% filter(x < gq_x[1])) %>%
  residuals() %>% raise_to_power(2) %>% sum()
sse2 = lm(y ~ x, data = gq_df %>% filter(x > gq_x[2])) %>%
  residuals() %>% raise_to_power(2) %>% sum()
ggplot(data = gq_df, aes(x = x, y = e)) +
geom_point(color = "darkslategrey", size = 2.75, alpha = 0.5) +
labs(x = "x", y = "u") +
theme_axes_math
```

---

## Візуалізація тесту Гольдфельда-Квандта

```{r, gq1b, echo = F, dev = "svg", fig.height = 4}
ggplot(data = gq_df, aes(
  x = x, y = e,
  color = cut(x, c(-Inf, gq_x, Inf)),
  alpha = cut(x, c(-Inf, gq_x, Inf)),
  shape = cut(x, c(-Inf, gq_x, Inf))
)) +
geom_vline(
  xintercept = gq_x,
  color = grey_mid,
  size = 0.25
) +
geom_point(size = 2.75) +
labs(x = "x", y = "u") +
scale_color_manual(values = c("darkslategrey", grey_mid, red_pink)) +
scale_shape_manual(values = c(19, 1, 19)) +
scale_alpha_manual(values = c(0.5, 0.8, 0.6)) +
theme_axes_math
```

$F_{375,\,375} = \dfrac{\color{#e64173}{\text{SSE}_2 = `r format(round(sse2, 1), nsmall = 0L, big.mark = ",")`}}{\color{#314f4f}{\text{SSE}_1 = `r format(round(sse1, 1), nsmall = 0L, big.mark = ",")`}} \approx `r format(round(sse2/sse1, 1), nsmall = 0L, big.mark = ",")` \implies$ *p*-value $< 0.001$

$\therefore$ Ми відхиляємо H~0~: $\sigma^2_1 = \sigma^2_2$ і робимо висновок про наявність статистично значущих доказів гетероскедастичності.

---

## Проблеми тесту Гольдфельда-Квандта

```{r, gq2a, echo = F, dev = "svg", fig.height = 4}
set.seed(12345)
# Data
gq2_df = tibble(
  x = runif(1e3, -3, 3),
  e = rnorm(1e3, 0, sd = 2 + x^2),
  y = 1 + 3 * x + e
)
# Quantiles
gq_x = quantile(gq2_df$x, probs = c(3/8, 5/8))
# Regressions
sse1b = lm(y ~ x, data = gq2_df %>% filter(x < gq_x[1])) %>%
  residuals() %>% raise_to_power(2) %>% sum()
sse2b = lm(y ~ x, data = gq2_df %>% filter(x > gq_x[2])) %>%
  residuals() %>% raise_to_power(2) %>% sum()
ggplot(data = gq2_df, aes(
  x = x, y = e
)) +
geom_point(color = "darkslategrey", size = 2.75, alpha = 0.5, shape = 19) +
labs(x = "x", y = "u") +
theme_axes_math
```

---

## Проблеми тесту Гольдфельда-Квандта

```{r, gq2b, echo = F, dev = "svg", fig.height = 4}
ggplot(data = gq2_df, aes(
  x = x, y = e,
  color = cut(x, c(-Inf, gq_x, Inf)),
  alpha = cut(x, c(-Inf, gq_x, Inf)),
  shape = cut(x, c(-Inf, gq_x, Inf))
)) +
geom_vline(
  xintercept = gq_x,
  color = grey_mid,
  size = 0.25
) +
geom_point(size = 2.75) +
labs(x = "x", y = "u") +
scale_color_manual(values = c("darkslategrey", grey_mid, red_pink)) +
scale_shape_manual(values = c(19, 1, 19)) +
scale_alpha_manual(values = c(0.5, 0.8, 0.6)) +
theme_axes_math
```

$F_{375,\,375} = \dfrac{\color{#e64173}{\text{SSE}_2 = `r format(round(sse2b, 1), nsmall = 0L, big.mark = ",")`}}{\color{#314f4f}{\text{SSE}_1 = `r format(round(sse1b, 1), nsmall = 0L, big.mark = ",")`}} \approx `r format(round(sse2b/sse1b, 1), nsmall = 0L, big.mark = ",")` \implies$ *p*-value $\approx `r round(pf(sse2b/sse1b, 375, 375, lower.tail = F), 3)`$

$\therefore$ Нам не вдається відхилити H~0~: $\sigma^2_1 = \sigma^2_2$, хоча гетероскедастичність очевидно присутня.

# Тест Уайта

## Тест Уайта

**Бройш і Паган (1981)** намагалися вирішити цю проблему конкретної функціональної форми гетероскедастичності.

- Регресія $e_i^2$ від $X = \left[ 1,\, x_1,\, x_2,\, \ldots,\, x_k \right]$ і перевірка значущості.
- Дозволяє даним показати, чи або як дисперсія $u_i$ корелює з $X$.
- Якщо $\sigma_i^2$ корелює з $X$, то ми маємо гетероскедастичність.

. . .

Однак ми насправді хочемо знати, чи

$$\sigma_1^2 = \sigma_2^2 = \cdots = \sigma_n^2$$

---

## Тест Уайта

Для досягнення цієї мети Гел Уайт скористався тим фактом, що ми можемо [замінити вимогу гомоскедастичності слабшим припущенням]{.hi}:

- **Старе припущення:** $\mathop{\text{Var}} \left( u_i \middle| X \right) = \sigma^2$

- **Нове припущення:** $u^2$ *некорельовані* з пояснювальними змінними (_тобто_, $x_j$ для всіх $j$), їхніми квадратами (_тобто_, $x_j^2$), а також взаємодії першого ступеня (_тобто_, $x_j x_h$).

. . .

Це нове припущення легше перевірити (*підказка:* регресія).

---

## Алгоритм тесту Уайта

1\. Регресія y на x~1~, x~2~, …, x~k~. Зберегти залишки e.

2\. Регресія квадратів залишків для всіх пояснювальних змінних, їхніх квадратів і взаємодій.

$$e^2 = \alpha_0 + \sum_{h=1}^k \alpha_h x_h + \sum_{j=1}^k \alpha_{k+j} x_j^2 + \sum_{\ell = 1}^{k-1} \sum_{m = \ell + 1}^k \alpha_{\ell,m} x_\ell x_m + v_i$$

3\. Запис R~e~^2^.

4\. Обчисліть тестову статистику для перевірки H~0~: $\alpha_p = 0$ для всіх $p\neq0$.

---

## Статистика тесту Уайта {.smaller}

$$ \text{LM} = n \times R_e^2 \qquad \text{де H}_0,\, \text{LM} \overset{\text{d}}{\sim} \chi_k^2 $$

де $R^2_e$ походить від регресії $e^2$ на пояснювальні змінні, їхні квадрати та їхні взаємодії.

$$ e^2 = \alpha_0 + \underbrace{\sum_{h=1}^k \alpha_h x_h}_{\text{Поясн. змінні}} + \underbrace{\sum_{j=1}^k \alpha_{k+j} x_j^2}_{\text{Квадратична форма}} + \underbrace{\sum_{\ell = 1}^{k-1} \sum_{m = \ell + 1}^k \alpha_{\ell,m} x_\ell x_m}_{\text{Взаємодія}} + v_i $$

::: {.callout-note}
$k$ (для нашого $\chi_k^2$) дорівнює кількості оцінених параметрів у наведеній вище регресії ($\alpha_j$), за винятком $\left( \alpha_0 \right)$.
:::

---

## Приклад тесту Уайта {.smaller}

*Приклад:* Розглянемо модель[^†] $y = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + \beta_3 x_3 + u$

[^†]: Щоб спростити позначення тут, я відкидаю індекси $i$.

**Крок 1:** Оцініть модель; отримати залишки $(e)$.

**Крок 2:** Регресія $e^2$ від пояснювальних змінних, квадратів та взаємодії.

$$
\begin{aligned}
  e^2 =
  &\alpha_0 + \alpha_1 x_1 + \alpha_2 x_2 + \alpha_3 x_3 + \alpha_4 x_1^2 + \alpha_5 x_2^2 + \alpha_6 x_3^2 \\
  &+ \alpha_7 x_1 x_2 + \alpha_8 x_1 x_3 + \alpha_9 x_2 x_3 + v
\end{aligned}
$$

Запишіть R^2^ із цього рівняння (назвіть його $R_e^2$).

**Крок 3:** Перевірте H~0~: $\alpha_1 = \alpha_2 = \cdots = \alpha_9 = 0$ за допомогою $\text{LM} = n R^2_e \overset{\text{d} }{\sim} \chi_9^2$.

---

## Приклад тесту Уайта

```{r, white1, echo = F, dev = "svg", fig.height = 4}
set.seed(12345)
# Data
bp_df = tibble(
  x = runif(1e3, -3, 3),
  e = rnorm(1e3, 0, sd = 2 + x^2),
  y = 1 + 3 * x + e
)

lm_bp1 = lm(residuals(lm(y ~ x, bp_df))^2 ~ 1 + bp_df$x) %>%
  summary() %$% r.squared %>% multiply_by(1e3)
lm_bp2 = lm(residuals(lm(y ~ x, bp_df))^2 ~ 1 + bp_df$x + I(bp_df$x^2)) %>%
  summary() %$% r.squared %>% multiply_by(1e3)

# The figure
ggplot(data = bp_df, aes(x = x, y = e)) +
geom_point(size = 2.75, color = "darkslategrey", alpha = 0.5) +
labs(x = "x", y = "u") +
theme_axes_math
```

Тест Уайта для цієї простої лінійної регресії.

$$
\begin{aligned}
 e_i^2 &= \hat{\alpha}_0 + \hat{\alpha}_1 x_{1i} \color{#e64173}{+ \hat{\alpha}_2 x^2_{1i}} & \widehat{\text{LM}} &= `r round(lm_bp2, 2)` &\mathit{p}\text{-value} < 0.001
\end{aligned}
$$

# Приклади

## Тестування гетероскедастичності {.smaller}

**Мета:** Оцінити зв’язок між стандартизованими результатами тестування (залежна змінна) і (1) співвідношенням студент/викладач і (2) доходом, _тобто_,

$$ \left(\text{Test score}\right)_i = \beta_0 + \beta_1 \text{Ratio}_{i} + \beta_2 \text{Income}_{i} + u_i \tag{1} $$

**Потенційна проблема:** Гетероскедастичність... і ми не знаємо $u_i$.

**Рішення:**

1. Оцініть модель $(1)$ за допомогою МНК.
2. Тест на гетероскедастичність.
   - Гольдфельд-Квандт
   - Уайта
   
---

## Тестування гетероскедастичності

Ми будемо використовувати дані тестування з набору даних `Caschool` в пакеті `Ecdat`.

::: {.panel-tabset}

### R

```{r, ex-data-r}
# Завантаження пакетів
library(pacman)
p_load(tidyverse, Ecdat)
# Відбір та перейменування змінних, створення нового набору даних
test_df  <- select(Caschool, test_score = testscr, ratio = str, income = avginc)
# Формат тіббл
test_df <- as_tibble(test_df)
# Перші 2 рядки
head(test_df, 2)
```

### Python

```{python, ex-data-py}
# Завантаження пакетів
import pandas as pd
from statsmodels.datasets import get_rdataset
# Завантаження даних
caschool = get_rdataset("Caschool", "Ecdat")
# Відбір та перейменування змінних
test_df = caschool.data[["testscr", "str", "avginc"]]
test_df.columns = ["test_score", "ratio", "income"]
# Перші 2 рядки
test_df.head(2)
```

:::

---

## Тестування гетероскедастичності {.smaller}

Почнемо з оцінки нашої моделі

$$ \left(\text{Test score}\right)_i = \beta_0 + \beta_1 \text{Ratio}_{i} + \beta_2 \text{Income}_{i} + u_i $$

::: {.panel-tabset}

### R

```{r, ex gq1 r}
# Оцінювання моделі
est_model = lm(test_score ~ ratio + income, data = test_df)
# Вивід результату
tidy(est_model) %>% knitr::kable()
```

### Python

```{python, ex gq1 p}
# Оцінювання моделі
from statsmodels.formula.api import ols
est_model = ols("test_score ~ ratio + income", data = test_df).fit()
# Вивід результату
est_model.summary().tables[1]
```

:::

---

## Тестування гетероскедастичності

Тепер давайте подивимося, що говорять залишки про гетероскедастичність

::: {.panel-tabset}

### R

```{r, ex gq2 r}
# Додамо залишки до даних
test_df$e = residuals(est_model)
```

### Python

```{python, ex gq2 p}
# Додамо залишки до даних
test_df["e"] = est_model.resid
```

:::

```{r, gq3, echo = F, dev = "svg", fig.height = 4.25}
# Plot residuals against income
plot1 = ggplot(data = test_df, aes(x = income, y = e)) +
  geom_point(size = 2.5, alpha = 0.5, color = red_pink) +
  labs(x = "Income", y = TeX("\\textit{e}")) +
  theme_axes_serif
# Plot residuals against student-teacher ratio
plot2 = ggplot(data = test_df, aes(x = ratio, y = e)) +
  geom_point(size = 2.5, alpha = 0.5, color = "darkslategrey") +
  labs(x = "Student-to-teacher ratio", y = TeX("\\textit{e}")) +
  theme_axes_serif
# The grid
gridExtra::grid.arrange(plot1, plot2, nrow = 1)
```

---

## Тестування гетероскедастичності: [G-Q]{.hi}

`income` виглядає потенційно гетероскедастичним; давайте перевіримо через Goldfeld-Quandt.

::: {.panel-tabset}

### R

```{r, ex gq4 r, eval = F}
# Впорядковуємо дані
test_df = arrange(test_df, income)
```

### Python

```{python, ex gq4 p, eval = F}
# Впорядковуємо дані
test_df = test_df.sort_values("income")
```

:::

---

## Тестування гетероскедастичності: [G-Q]{.hi}

`income` виглядає потенційно гетероскедастичним; давайте перевіримо через Goldfeld-Quandt.

::: {.panel-tabset}

### R

```{r, ex gq5 r, eval = F}
# Впорядковуємо дані
test_df = arrange(test_df, income)
# Переоцінюємо модель для останніх і перших 158 спостережень
est_model1 = lm(test_score ~ ratio + income, data = tail(test_df, 158))
est_model2 = lm(test_score ~ ratio + income, data = head(test_df, 158))
```

### Python

```{python, ex gq5 p, eval = F}
# Впорядковуємо дані
test_df = test_df.sort_values("income")
# Переоцінюємо модель для останніх і перших 158 спостережень
est_model1 = ols("test_score ~ ratio + income", data = test_df.tail(158)).fit()
est_model2 = ols("test_score ~ ratio + income", data = test_df.head(158)).fit()
```

:::

---

## Тестування гетероскедастичності: [G-Q]{.hi}

`income` виглядає потенційно гетероскедастичним; давайте перевіримо через Goldfeld-Quandt.

::: {.panel-tabset}

### R

```{r, ex gq6 r}
# Впорядковуємо дані
test_df = arrange(test_df, income)
# Переоцінюємо модель для останніх і перших 158 спостережень
est_model1 = lm(test_score ~ ratio + income, data = tail(test_df, 158))
est_model2 = lm(test_score ~ ratio + income, data = head(test_df, 158))
# Беремо залишки від кожної регресії
e_model1 = residuals(est_model1)
e_model2 = residuals(est_model2)
```

### Python

```{python, ex gq6 p}
# Впорядковуємо дані
test_df = test_df.sort_values("income")
# Переоцінюємо модель для останніх і перших 158 спостережень
est_model1 = ols("test_score ~ ratio + income", data = test_df.tail(158)).fit()
est_model2 = ols("test_score ~ ratio + income", data = test_df.head(158)).fit()
# Беремо залишки від кожної регресії
e_model1 = est_model1.resid
e_model2 = est_model2.resid
```

:::

---

## Тестування гетероскедастичності: [G-Q]{.hi}

`income` виглядає потенційно гетероскедастичним; давайте перевіримо через Goldfeld-Quandt.

::: {.panel-tabset}

### R

```{r, ex gq7 r}
# Впорядковуємо дані
test_df = arrange(test_df, income)
# Переоцінюємо модель для останніх і перших 158 спостережень
est_model1 = lm(test_score ~ ratio + income, data = tail(test_df, 158))
est_model2 = lm(test_score ~ ratio + income, data = head(test_df, 158))
# Розраховуємо SSE для кожної регресії
sse_model1 = sum(e_model1^2)
sse_model1
sse_model2 = sum(e_model2^2)
sse_model2
```

### Python

```{python, ex gq7 p}
# Впорядковуємо дані
test_df = test_df.sort_values("income")
# Переоцінюємо модель для останніх і перших 158 спостережень
est_model1 = ols("test_score ~ ratio + income", data = test_df.tail(158)).fit()
est_model2 = ols("test_score ~ ratio + income", data = test_df.head(158)).fit()
# Розраховуємо SSE для кожної регресії
sse_model1 = (e_model1**2).sum()
sse_model1
sse_model2 = (e_model2**2).sum()
sse_model2
```

:::

---

## Тестування гетероскедастичності: [G-Q]{.hi} {.smaller}

Розраховуємо статистику:

$F_{n^\star-k,\,n^\star-k} = \dfrac{\text{SSE}_2}{\text{SSE}_1}$

. . .

$\approx\dfrac{`r round(sse_model2, 2) %>% format(big.mark = ",")`}{`r round(sse_model1, 2) %>% format(big.mark = ",")`}$

. . .

$\approx`r round(sse_model2/sse_model1, 2)`$

. . .

$F_{158-3,\,158-3}$

. . .

::: {.panel-tabset}

### R

```{r, ex gq8 r}
# G-Q статистика
(f_gq = sse_model2/sse_model1)
```

```{r, ex gq9 r}
# p-value
pf(q = f_gq, df1 = 158-3, df2 = 158-3, lower.tail = F)
```

### Python

```{python, ex gq8 p}
# G-Q статистика
f_gq = sse_model2/sse_model1
f_gq
```

```{python, ex gq9 p}
# p-value
from scipy.stats import f
f.sf(f_gq, 158-3, 158-3)
```

:::

---

## Тестування гетероскедастичності: [G-Q]{.hi}

Критерій статистики Гольдфельда-Квандта та його розподіл

```{r, ex gq10, echo = F, dev = "svg", fig.height = 5}
ggplot(data = tibble(x = c(0, f_gq * 1.3)), aes(x)) +
  geom_area(
    stat = "function", fun = df, args = list(df1 = 158, df2 = 158),
    fill = "darkslategrey", alpha = 0.05
  ) +
  geom_area(
    stat = "function", fun = df, args = list(df1 = 158, df2 = 158),
    fill = red_pink, alpha = 0.85,
    xlim = c(f_gq, f_gq * 1.3)
  ) +
  stat_function(
    fun = df, args = list(df1 = 158, df2 = 158), n = 1e3,
    color = "darkslategrey"
  ) +
  geom_vline(xintercept = f_gq, color = grey_dark, size = 0.5, linetype = "dotted") +
  annotate(
    "text", x = f_gq, y = 5 * df(f_gq, df1 = 158, df2 = 158),
    label = TeX("$\\widehat{F}=1.53$"),
    family = "MathJax_Main", size = 7
  ) +
  labs(x = "x", y = "f") +
  theme_axes_math
```

---

## Тестування гетероскедастичності: [G-Q]{.hi}

Зібравши все разом:

H~0~: $\sigma_1^2 = \sigma_2^2$ *проти* H~A~: $\sigma_1^2 \neq \sigma_2^2$

Статистика тесту Голдфельда-Квандта: $F \approx `r round(f_gq, 2)`$

*p*-значення $\approx `r round(pf(q = f_gq, df1 = 158-3, df2 = 158-3, lower.tail = F), 5)`$

∴ Відхилити H~0~ (*p*-значення менше 0,05).

**Висновок:** Є статистично значущі докази того, що $\sigma_1^2 \neq \sigma_2^2$. Таким чином, ми знаходимо статистично значущі докази гетероскедастичності (на 5-відсотковому рівні значущості).

---

## Тестування гетероскедастичності: [G-Q]{.hi}

Що, якби ми вирішили зосередитися на `ratio`?

. . .

::: {.panel-tabset}

### R

```{r, ex gq11 r}
# Впорядкування
test_df = arrange(test_df, ratio)
# Моделі
est_model3 = lm(test_score ~ ratio + income, data = tail(test_df, 158))
est_model4 = lm(test_score ~ ratio + income, data = head(test_df, 158))
# Залишки
e_model3 = residuals(est_model3)
e_model4 = residuals(est_model4)
# SSE
(sse_model3 = sum(e_model3^2))
(sse_model4 = sum(e_model4^2))
```

### Python

```{python, ex gq11 p}
# Впорядкування
test_df = test_df.sort_values("ratio")
# Моделі
est_model3 = ols("test_score ~ ratio + income", data = test_df.tail(158)).fit()
est_model4 = ols("test_score ~ ratio + income", data = test_df.head(158)).fit()
# Залишки
e_model3 = est_model3.resid
e_model4 = est_model4.resid
# SSE
sse_model3 = (e_model3**2).sum()
sse_model4 = (e_model4**2).sum()
sse_model3, sse_model4
```

:::

---

## Тестування гетероскедастичності: [G-Q]{.hi}

$F_{n^\star-k,\,n^\star-k} = \dfrac{\text{SSE}_4}{\text{SSE}_3} \approx\dfrac{`r round(sse_model4, 2) %>% format(big.mark = ",")`}{`r round(sse_model3, 2) %>% format(big.mark = ",")`} \approx`r round(sse_model4 / sse_model3, 2)`$

яке має *p*-значення приблизно `r round(pf(sse_model4 / sse_model3, 158-3, 158-3, lower.tail = F), 4)`.

. . .

∴ Ми б не змогли відхилити H~0~, дійшовши висновку, що нам не вдалося знайти статистично значущі докази гетероскедастичності.

---

## Тестування гетероскедастичності: [Уайт]{.hi}

Давайте перевіримо ту саму модель і дані за допомогою тесту Уайта.

*Нагадуємо:* ми зберегли наші залишки як `e` у нашому наборі даних, _тобто_,

::: {.panel-tabset}

### R

```{r, white recall r, eval = F}
# Модель
est_model = lm(test_score ~ ratio + income, data = test_df)
# Залишки
test_df$e = residuals(est_model)
```

### Python

```{python, white recall p, eval = F}
# Модель
est_model = ols("test_score ~ ratio + income", data = test_df).fit()
# Залишки
test_df["e"] = est_model.resid
```

:::

---

## Тестування гетероскедастичності: [Уайт]{.hi} {.smaller}

[**Тест Уайта** додає змінні в квадраті та взаємодії]{.pink} до початкової специфікації регресії (права сторона)

$$
\begin{align}
 \color{#314f4f}{u_{i}^2} =& \color{#314f4f}{\alpha_{0} + \alpha_{1} \text{Ratio}_{i} + \alpha_{2} \text{Income}_{i} } \\
 &+ \color{#e64173}{\alpha_{3} \text{Ratio}_{i}^2 + \alpha_{4} \text{Income}_{i}^2 + \alpha_{5} \text{Ratio}_{i}\times\text{Income}_{i}} \\
 &+ \color{#314f4f}{w_{i}}
\end{align}
$$

[**Тест Уайта**]{.pink} перевіряє нульову гіпотезу
<br>[H~0~:]{.pink} $\color{#e64173}{\alpha_1 = \alpha_2 = \alpha_3 = \alpha_4 = \alpha_5 = 0}$

::: {.callout-note}
`R` та `Python` має нотацію квадратичних форм і взаємодій у `lm()`/`ols()`:

- **Квадратні форми** використовують `I()`, _наприклад_, `lm(y ~ I(x^2))`

- **Взаємодії** використовують `:` між змінними, _наприклад_, `lm(y ~ x1:x2)`

*Приклад:* Регресія `y` до квадратичного від `x1` і `x2`:

```{r, ex quad, eval = F}
lm(y ~ x1 + x2 + I(x1^2) + I(x2^2) + x1:x2, data = pretend_df)
```
:::

---

## Тестування гетероскедастичності: [Уайт]{.hi}

**Крок 1:** Регресія $e_i^2$ від 1^ї^ ступені, 2^ї^ ступінь і взаємодії

::: {.panel-tabset}

### R

```{r, ex white0 r, eval = F}
# Модель з квадратами і взаємодіями 
white_model = lm(
  I(e^2) ~ ratio + income + I(ratio^2) + I(income^2) + ratio:income,
  data = test_df
)
```

### Python

```{python, ex white0 p, eval = F}
# Модель з квадратами і взаємодіями
white_model = ols("I(e**2) ~ ratio + income + I(ratio**2) + I(income**2) + ratio:income", data = test_df).fit()
```

:::

---

## Тестування гетероскедастичності: [Уайт]{.hi}

**Крок 2:** Зберіть $R_e^2$ з регресії.

::: {.panel-tabset}

### R

```{r, ex white1 r, eval = T}
# Модель з квадратами і взаємодіями 
white_model = lm(
  I(e^2) ~ ratio + income + I(ratio^2) + I(income^2) + ratio:income,
  data = test_df
)
# R-squared
(white_r2 = summary(white_model)$r.squared)
```

### Python

```{python, ex white1 p, eval = T}
# Модель з квадратами і взаємодіями
white_model = ols("I(e**2) ~ ratio + income + I(ratio**2) + I(income**2) + ratio:income", data = test_df).fit()
# R-squared
white_r2 = white_model.rsquared
white_r2
```

:::

---

## Тестування гетероскедастичності: [Уайт]{.hi}

**Крок 3:** Обчисліть статистику тесту Уайта $\text{LM} = n \times R_e^2 \approx 420 \times `r round(white_r2, 3)`$

::: {.panel-tabset}

### R

```{r, ex white3 r, eval = T}
# Модель з квадратами і взаємодіями 
white_model = lm(
  I(e^2) ~ ratio + income + I(ratio^2) + I(income^2) + ratio:income,
  data = test_df
)
# R-squared
white_r2 = summary(white_model)$r.squared
# White test statistic
(white_stat = 420 * white_r2)
```

### Python

```{python, ex white3 p, eval = T}
# Модель з квадратами і взаємодіями
white_model = ols("I(e**2) ~ ratio + income + I(ratio**2) + I(income**2) + ratio:income", data = test_df).fit()
# R-squared
white_r2 = white_model.rsquared
# White test statistic
white_stat = 420 * white_r2
white_stat
```

:::

---

## Тестування гетероскедастичності: [Уайт]{.hi}

**Крок 4:** Обчисліть *p*-значення (де $\text{LM} \overset{d}{\sim} \chi_k^2$); тут $k=5$

::: {.panel-tabset}

### R

```{r, ex white4 r, eval = T}
# Модель з квадратами і взаємодіями 
white_model = lm(
  I(e^2) ~ ratio + income + I(ratio^2) + I(income^2) + ratio:income,
  data = test_df
)
# R-squared
white_r2 = summary(white_model)$r.squared
# White test statistic
white_stat = 420 * white_r2
# p-value
pchisq(q = white_stat, df = 5, lower.tail = F)
```

### Python

```{python, ex white4 p, eval = T}
# Модель з квадратами і взаємодіями
white_model = ols("I(e**2) ~ ratio + income + I(ratio**2) + I(income**2) + ratio:income", data = test_df).fit()
# R-squared
white_r2 = white_model.rsquared
# White test statistic
white_stat = 420 * white_r2
# p-value
from scipy.stats import chi2
chi2.sf(white_stat, 5)
```

:::

---

## Тестування гетероскедастичності: [Уайт]{.hi} {.smaller}

Тепер все разом:

. . .

H~0~: $\alpha_1 = \alpha_2 = \alpha_3 = \alpha_4 = \alpha_5 = 0$ *vs.* H~A~: $\alpha_i \neq 0$ для $i \in \{1,\, 2,\,\ldots,\, 5\}$

. . .

$$
\begin{align}
u_i^2 =& \alpha_0 + \alpha_1 \text{Ratio}_i + \alpha_2 \text{Income}_i \\
&+ \alpha_3 \text{Ratio}_i^2 + \alpha_4 \text{Income}_i^2 \\
&+ \alpha_5 \text{Ratio}_i \times \text{Income}_i + w_i
\end{align}
$$

. . .

Наша статистика білого тесту: $\text{LM} = n \times R_e^2 \approx 420 \times `r round(white_r2, 3)` \approx `r round(white_stat, 2)`$

. . .

За розподілом $\chi_5^2$ цей $\widehat{\text{LM}}$ має *p*-значення менше 0,001.

. . .

∴ Ми [відхиляємо H~0~]{.hi} і робимо висновок про наявність [статистично значущих доказів гетероскедастичності]{.hi} (на 5-відсотковому рівні).

---

## Тестування гетероскедастичності: [Уайт]{.hi}

Статистика тесту Уайта та її розподіл

```{r, ex white plot, echo = F, dev = "svg", fig.height = 5}
ggplot(data = tibble(x = c(0, white_stat * 1.1)), aes(x)) +
  geom_area(
    stat = "function", fun = dchisq, args = list(df = 5),
    fill = "darkslategrey", alpha = 0.05
  ) +
  geom_area(
    stat = "function", fun = dchisq, args = list(df = 5),
    fill = red_pink, alpha = 0.85,
    xlim = c(white_stat, white_stat * 1.1)
  ) +
  stat_function(
    fun = dchisq, args = list(df = 5), n = 1e3,
    color = "darkslategrey"
  ) +
  geom_vline(xintercept = white_stat, color = grey_dark, size = 0.5, linetype = "dotted") +
  annotate(
    "text", x = white_stat, y = 0.01,
    label = TeX("$\\widehat{LM}=30.8$"),
    family = "MathJax_Main", size = 7,
    hjust = 0.5
  ) +
  labs(x = "x", y = "f") +
  theme_axes_math
```

# Життя з гетероскедастичністю

## Життя з гетероскедастичністю {.smaller}

При наявності гетероскедастичності МНК є

- все ще [незміщеною]{.hi}
- [більше не найефективніший]{.hi} незміщений лінійний оцінювач

У середньому ми отримуємо правильну відповідь, але з більшим шумом (меншою точністю).
<br> *Також:* Наші стандартні помилки є зміщеними

. . .

**Що можна зробити:**

1. Перевірте [специфікацію]{.hi} регресії .
2. Знайдіть новий, більш ефективний [незміщений оцінювач]{.hi} для $\beta_j$.
3. Жити з неефективністю МНК; знайти [новий засіб оцінки дисперсії]{.hi}.
   - Стандартні помилки
   - Довірчі інтервали
   - Перевірка гіпотез

---

## Неправильна специфікація

Як ми вже обговорювали, специфікація вашої моделі регресії має велике значення для неупередженості та ефективності вашого оцінювача.

**Відповідь \#1:** Переконайтеся, що ваша специфікація не викликає гетероскедастичності.

---

## Неправильна специфікація {.smaller}

*Приклад:* Нехай залежність у ГС буде $$ y_i = \beta_0 + \beta_1 x_i + \beta_2 x_i^2 + u_i $$

з $\mathop{\boldsymbol{E}}\left[ u_i \middle| x_i \right] = 0$ і $\mathop{\text{Var}} \left( u_i \middle| x_i \right) = \sigma^2$.

Однак ми пропускаємо $x^2$ і оцінюємо $$ y_i = \gamma_0 + \gamma_1 x_i + w_i $$

Тоді $$ w_i = u_i + \beta_2 x_i^2 \implies \mathop{\text{Var}} \left( w_i \right) = f(x_i) $$

_Тобто_, дисперсія $w_i$ змінюється систематично з $x_i$ (гетероскедастичність).

---

## Неправильна специфікація

```{r, spec data, include = F}
# Set the seed
set.seed(1234)
# Generate data
spec_df <- tibble(x = runif(1e3, 0, 3), y = exp(0.5 + 0.6 * x + rnorm(1e3, sd = 0.3)))
# Add residuals ('w': wrong specification; 'c': correct specification)
spec_df %<>% mutate(
  e_w = lm(y ~ x, data = spec_df) %>% residuals(),
  e_c = lm(log(y) ~ x, data = spec_df) %>% residuals()
)
```

[Істина:]{.pink} $\color{#e64173}{\log\left(y_i\right) = \beta_0 + \beta_1 x_i + u_i}$ 

[Хибна:]{.slate} $\color{#314f4f}{y_i = \beta_0 + \beta_1 x_i + v_i}$

```{r, spec plot1, echo = F, dev = "svg", fig.height = 5}
ggplot(data = spec_df, aes(x = x)) +
  geom_point(aes(y = e_w), color = "darkslategrey", size = 2.75, alpha = 0.5, shape = 16) +
  geom_point(aes(y = e_c), color = red_pink, size = 2.5, alpha = 0, shape = 19) +
  labs(x = "x", y = "e") +
  theme_axes_math
```

---

## Неправильна специфікація

[Істина:]{.pink} $\color{#e64173}{\log\left(y_i\right) = \beta_0 + \beta_1 x_i + u_i}$ 

[Хибна:]{.slate} $\color{#314f4f}{y_i = \beta_0 + \beta_1 x_i + v_i}$

```{r, spec plot2, echo = F, dev = "svg", fig.height = 5}
ggplot(data = spec_df, aes(x = x)) +
  geom_point(aes(y = e_w), color = "darkslategrey", size = 2.75, alpha = 0.25, shape = 1) +
  geom_point(aes(y = e_c), color = red_pink, size = 2.5, alpha = 0.5, shape = 19) +
  labs(x = "x", y = "e") +
  theme_axes_math
```

---

## Неправильна специфікація {.smaller}

**Проблема неправильної специфікації:** Неправильна специфікація регресійної моделі може спричинити гетероскедастичність (серед інших проблем).

. . .

**Рішення:** 💡 Зробіть це правильно (_наприклад_, не пропускайте $x^2$).

. . .

**Нові проблеми:**

- Ми часто не знаємо *правильної* специфікації.
- Ми хотіли б більш формального процесу вирішення проблеми гетероскедастичності.

. . .

**Висновок: ** Специфікація часто не «вирішить» промлему гетероскедастичності.
<br> Однак правильне визначення моделі все одно є дуже важливим.

---

## Зважений МНК {.smaller}

Зважений МНК (WLS) представляє інший підхід.

**Відповідь \#2:** Підвищення ефективності шляхом зважування наших спостережень.

. . .

Нехай істинне рівняння генеральної сукупності буде

$$
\begin{align}
   y_i = \beta_0 + \beta_1 x_{i} + u_i \tag{1}
\end{align}
$$

з $u_i \sim \mathop{N} \left( 0,\, \sigma_i^2 \right)$.

. . .

Тепер перетворіть $(1)$, поділивши дані кожного спостереження на $\sigma_i$, _тобто_,

$$
\begin{align}
   \dfrac{y_i}{\sigma_i} &= \beta_0 \dfrac{1}{\sigma_i} + \beta_1 \dfrac{x_{i}}{\sigma_i} + \dfrac{u_i}{\sigma_i} \tag{ 2}
\end{align}
$$

---

## Зважений МНК

$$
\begin{align}
  y_i &= \beta_0 + \beta_1 x_{i} + u_i \tag{1} \\[1em]
  \dfrac{y_i}{\sigma_i} &= \beta_0 \dfrac{1}{\sigma_i} + \beta_1 \dfrac{x_{i}}{\sigma_i} + \dfrac{u_i}{\sigma_i} \tag{2}
\end{align}
$$

Тоді як $(1)$ є гетероскедастичним, а $\color{#e64173}{(2)}$ [гомоскедастичним]{.hi}.

∴ МНК ефективний і незміщенний для оцінки $\beta_k$ у $(2)$!

. . .

Чому $(2)$ є гомоскедастичним?

$\mathop{\text{Var}} \left( \dfrac{u_i}{\sigma_i} \middle| x_i \right) =$
 $\dfrac{1}{\sigma_i^2} \mathop{\text{Var}} \left( u_i \middle| x_i \right) =$
 $\dfrac{1}{\sigma_i^2} \sigma_i^2 =$
 $1$

---

## Зважений МНК

ЗМНК — це чудово, але нам потрібно знати $\sigma_i^2$, що, як правило, малоймовірно.

Ми можемо *трохи* послабити цю вимогу — замість того, щоб вимагати

1. $\mathop{\text{Var}} \left( u_i | x_i \right) = \sigma_i^2 = \sigma^2 h(x_i)$

2. Ми знаємо $h(x)$.

. . .

Як і раніше, ми трансформуємо нашу гетероскедастичну модель у гомоскедастичну модель. Цього разу ми ділимо дані кожного спостереження[^†] на $\sqrt{h(x_i)}$.

[^†]: Розділіть *усі* дані на $\sqrt{h(x_i)}$, включно з intercept.

---

## Зважений МНК

$$
\begin{align}
  y_i &= \beta_0 + \beta_1 x_{i} + u_i \tag{1} \\[1em]
  \dfrac{y_i}{\sqrt{h(x_i)}} &= \beta_0 \dfrac{1}{\sqrt{h(x_i)}} + \beta_1 \dfrac{x_{i}}{\sqrt{h(x_i)}} + \dfrac{u_i}{\sqrt{h(x_i)}} \tag{2}
\end{align}
$$

з $\mathop{\text{Var}} \left( u_i | x_i \right) = \sigma^2 h(x_i)$.

. . .

Тепер давайте перевіримо, що $(2)$ справді є гомоскедастичним.

$\mathop{\text{Var}} \left( \dfrac{u_i}{\sqrt{h(x_i)}} \middle| x_i \right) =$
  $\dfrac{1}{h(x_i)} \mathop{\text{Var}} \left( u_i \middle| x_i \right) =$
  $\dfrac{1}{h(x_i)} \sigma^2 h(x_i) =$
  $\color{#e64173}{\sigma^2}$

[гомоскедастичність!]{.hi}

---

## Зважений МНК {.smaller}

Оцінювач [WLS]{.hi} — це спеціальний клас оцінок [узагальнених методів найменших квадратів]{.hi} (GLS), зосереджених на гетероскедастичності.

. . .

$$
  y_i = \beta_0 + \beta_1 x_{1i} + u_i \quad \color{#e64173}{\text{ vs. }} \quad
  \dfrac{y_i}{\sigma_i} = \beta_0 \dfrac{1}{\sigma_i} + \beta_1 \dfrac{x_{1i}}{\sigma_i} + \dfrac{u_i}{\sigma_i}
$$

*Примітки:*

1. WLS **перетворює** гетероскедастичну модель на гомоскедастичну модель.
2. **Зважування:** WLS знижує вагу спостережень із вищою дисперсією $u_i$.
3. **Значна вимога:** WLS вимагає, щоб ми *знали* $\sigma_i^2$ для кожного спостереження.
4. WLS, як правило, **неможливий**. *Доступний* GLS (FGLS) пропонує рішення.
5. Згідно з припущеннями: WLS є **найкращим лінійним незміщеним оцінювачем**.

---

## Гетероскедастично-робасті стандартні помилки {.smaller}

**Heteroskedasticity-robust standard errors**

**Відповідь \#3:**

- Ігнорувати неефективність OLS (при наявності гетероскедастичності).
- Зосередьтеся на [незміщеності оцінки стандартних помилок]{.hi}.
- Як результат: правильний висновок.

. . .

**Q:** Що таке стандартна помилка?
<br>**A:** [стандартне відхилення розподілу оцінювача]{.hi}.

. . .

Оцінювачі (наприклад, $\hat{\beta}_1$) є випадковими величинами, тому вони мають розподіл.

Стандартні помилки дають нам уявлення про те, наскільки варіативним є наш оцінювач.

---

## Гетероскедастично-робасті стандартні помилки {.smaller}

*Нагадуємо*: ми можемо записати оцінку МНК для $\beta_1$ як

$$ \hat{\beta}_1 = \beta_1 + \dfrac{\sum_i \left( x_i - \overline{x} \right) u_i}{\sum_i \left( x_i - \overline{x} \right)^2} = \beta_1 + \dfrac{\sum_i \left( x_i - \overline{x} \right) u_i}{\text{SST}_x} \tag{3} $$

. . .

Нехай $\mathop{\text{Var}} \left( u_i \middle| x_i \right) = \sigma_i^2$.

. . .

Ми можемо використовувати $(3)$, щоб записати дисперсію $\hat{\beta}_1$, _тобто_,

$$ \mathop{\text{Var}} \left( \hat{\beta}_1 \middle| x_i \right) = \dfrac{\sum_i \left( x_i - \overline{x} \right)^2 \sigma_i^2}{\text{SST}_x^2} \tag{4} $$

---

## Гетероскедастично-робасті стандартні помилки {.smaller}

Якщо ми хочемо отримати незміщену оцінку стандартних помилок, нам потрібна незміщена оцінка

$$ \dfrac{\sum_i \left( x_i - \overline{x} \right)^2 \sigma_i^2}{\text{SST}_x^2} $$

Наш старий друг Гел Уайт надав такий оцінювач:[^2]

$$ \widehat{\mathop{\text{Var}}} \left( \hat{\beta}_1 \right) = \dfrac{\sum_i \left( x_i - \overline{x} \right)^2 e_i ^2}{\text{SST}_x^2} $$

де $e_i$ походить від регресії МНК

[^2]: Це специфічне рівняння для простої лінійної регресії.

---

## Гетероскедастично-робасті стандартні помилки {.smaller}

Наші стійкі до гетероскедастичності оцінки для стандартної помилки $\beta_j$.

[Випадок 1]{.hi} Проста лінійна регресія, $y_i = \beta_0 + \beta_1 x_i + u_i$

$$ \widehat{\mathop{\text{Var}}} \left( \hat{\beta}_1 \right) = \dfrac{\sum_i \left( x_i - \overline{x} \right)^2 e_i^2}{\text{SST}_x^2} $$

[Випадок 2]{.hi} Множинна лінійна регресія, $y_i = \beta_0 + \beta_1 x_{1i} + \cdots + \beta_k x_{ki} + u_i$

$$ \widehat{\mathop{\text{Var}}} \left( \hat{\beta}_j \right) = \dfrac{\sum_i \hat{r}_{ij}^2 e_i^2}{\text{SST}_{x_j^2}} $$

де $\hat{r}_{ij}$ позначає $i$ залишок від регресії $x_j$ для всіх інших пояснювальних змінних.

---

## Гетероскедастично-робасті стандартні помилки

З цими стандартними помилками ми можемо повернутися до правильного статистичного висновку

_Наприклад_, ми можемо оновити нашу попередню статистичну формулу $t$ за допомогою наших нових стійких до гетероскедастичності стандартних помилок.

$$ t = \dfrac{\text{Estimate} - \text{Hypothesized value}}{\text{Standard error}} $$

---

## Гетероскедастично-робасті стандартні помилки {.smaller}

**Примітки**

- Ми все ще використовуємо [OLS оцінки для]{.hi} $\color{#e64173}{\beta_j}$
- Наші робасті стандартні помилки використовують [інший оцінювач]{.hi}.
- Гомоскедастичність
   - Звичайна оцінка дисперсії OLS більш ефективна.
   - Het.-robust все ще незміщена
- Гетероскедастичність
   - Звичайна оцінка дисперсії МНК є зміщеною.
   - Het.-робастна оцінка дисперсії є незміщеною.

---

## Гетероскедастично-робасті стандартні помилки

Ці стандартні помилки мають багато назв

- Heteroskedasticity-robust standard errors
- Het.-robust standard errors
- White standard errors
- Eicker-White standard errors
- Huber standard errors
- Eicker-Huber-White standards errors
- (деякі ігші комбінації Eicker, Huber, and White)

# Приклад: Специфікація моделі

## Приклад

Повертаємось до нашого датасету…

::: {.panel-tabset}

### R

```{r, ex test data r}
library(pacman)
p_load(tidyverse, Ecdat)

test_df <- Caschool %>% select(
  test_score = testscr, ratio = str, income = avginc, enrollment = enrltot
) %>% as_tibble()

head(test_df, 2)
```

### Python

```{python, ex test data p}
import pandas as pd
from statsmodels.datasets import get_rdataset

caschool = get_rdataset("Caschool", "Ecdat")

test_df = caschool.data[["testscr", "str", "avginc", "enrltot"]]
test_df.columns = ["test_score", "ratio", "income", "enrollment "]

test_df.head(2)
```

:::

---

## Приклад: Специфікація моделі

Ми виявили значні докази гетероскедастичності.

Давайте перевіримо, чи це сталося через неправильну специфікацію нашої моделі.

---

## Приклад: Специфікація моделі

Model[1]{.sub}: $\text{Score}_i = \beta_0 + \beta_1 \text{Ratio}_i + \beta_2 \text{Income}_i + u_i$

```{r, ex spec1, echo = F, dev = "svg", fig.height = 4.75}
# Model 1: test ~ ratio + income
test_df %<>% mutate(e1 = lm(test_score ~ ratio + income, data = test_df) %>% residuals())
# Plot
ggplot(data = test_df, aes(x = income, y = e1)) +
geom_point(size = 3, alpha = 0.5, color = red_pink) +
labs(x = "Income", y = TeX("\\textit{e}")) +
theme_axes_serif
```

---

## Приклад: Специфікація моделі

Model[2]{.sub}: $\log\left(\text{Score}_i\right) = \beta_0 + \beta_1 \text{Ratio}_i + \beta_2 \text{Income}_i + u_i$


```{r, ex spec2, echo = F, dev = "svg", fig.height = 4.75}
# Model 1: test ~ ratio + income
test_df %<>% mutate(e2 = lm(log(test_score) ~ ratio + income, data = test_df) %>% residuals())
# Plot
ggplot(data = test_df, aes(x = income)) +
geom_point(aes(y = e2), size = 3, alpha = 0.5, color = red_pink) +
labs(x = "Income", y = TeX("\\textit{e}")) +
theme_axes_serif
```

---

## Приклад: Специфікація моделі

Model[3]{.sub}: $\log\left(\text{Score}_i\right) = \beta_0 + \beta_1 \text{Ratio}_i + \beta_2 \log\left(\text{Income}_i\right) + u_i$

```{r, ex spec3, echo = F, dev = "svg", fig.height = 4.75}
# Model 1: test ~ ratio + income
test_df %<>% mutate(e3 = lm(log(test_score) ~ ratio + log(income), data = test_df) %>% residuals())
# Plot
ggplot(data = test_df, aes(x = log(income))) +
geom_point(aes(y = e3), size = 3, alpha = 0.5, color = red_pink) +
labs(x = "Log(Income)", y = TeX("\\textit{e}")) +
theme_axes_serif
```

---

## Приклад: Специфікація моделі {.smaller}

Давайте перевіримо цю нову специфікацію за допомогою тесту Уайта на гетероскедастичність.

Model[3]{.sub}: $\log\left(\text{Score}_i\right) = \beta_0 + \beta_1 \text{Ratio}_i + \beta_2 \log\left(\text{Income}_i\right) + u_i$

```{r, ex spec test, include = F}
white_r2_spec <- lm(e3^2 ~
  ratio * log(income) + I(ratio^2) + I(log(income)^2),
  data = test_df
) %>% summary() %$% r.squared
white_stat_spec <- white_r2_spec %>% multiply_by(420)
```

. . .

Регресія для тесту Уайта

. . .

$$
\begin{align}
  e_i^2 = &\alpha_0 + \alpha_1 \text{Ratio}_i + \alpha_2 \log\left(\text{Income}_i\right) + \alpha_3 \text{Ratio}_i^2 + \alpha_4 \left(\log\left(\text{Income}_i\right)\right)^2 \\
  &+ \alpha_5 \left(\text{Ratio}_i\times\log\left(\text{Income}_i\right)\right) + v_i
\end{align}
$$

. . .

 $R_e^2\approx`r round(white_r2_spec, 3)`$
і тестова статистика
 $\widehat{\text{LM}} = n\times R_e^2 \approx `r round(white_stat_spec, 1)`$.

. . .

H~0~ $\text{LM}$ розподілена як $\chi_5^2$ $\implies$ *p*-value $\approx$ `r pchisq(white_stat_spec, 5, lower.tail = F) %>% round(3)`.

. . .

∴ [Відхиляємо H~0~]{.hi} [Висновок:]{.hi}  Існують статистично значущі докази гетероскедастичності на п’ятивідсотковому рівні.

---

## Приклад: Специфікація моделі

Гаразд, ми спробували скорегувати нашу специфікацію, але все ще є докази гетероскедастичності.

**Далі:** Загалом, ви звернетеся до стійких до гетероскедастичності стандартних помилок.

- OLS все ще неупереджений для [коефіцієнтів]{.hi} ($\beta_j$')

- Стійкі до гетероскедастичності стандартні помилки є незміщеними для [стандартних помилок]{.hi} $\hat{\beta}_j$, _тобто_, $\sqrt{\mathop{\text{Var}} \left( \hat{\beta}_j \right)}$.

# Приклад: Het.-robust standard errors

## Приклад: Het.-robust standard errors

Повертаємось до моделі

$$ \text{Score}_i = \beta_0 + \beta_1 \text{Ratio}_i + \beta_2 \text{Income}_i + u_i $$

Ми можемо використовувати пакет `fixest` у `R` для обчислення стандартних помилок.

Або ми можемо використовувати `statsmodels` у `Python`.

---

## Приклад: Het.-robust standard errors

$$ \text{Score}_i = \beta_0 + \beta_1 \text{Ratio}_i + \beta_2 \text{Income}_i + u_i $$

::: {.panel-tabset}

### R

1\. Побудуйте регресію за допомогою `lm()`

```{r, fixest1 r}
test_reg <- lm(test_score ~ ratio + income, data = test_df)
```

### Python

1\. Побудуйте регресію за допомогою `statsmodels`

```{python, fixest1 p}
import statsmodels.formula.api as smf

test_reg = smf.ols("test_score ~ ratio + income", data = test_df).fit()
```

:::

---

## Приклад: Het.-robust standard errors {.smaller}

$$ \text{Score}_i = \beta_0 + \beta_1 \text{Ratio}_i + \beta_2 \text{Income}_i + u_i $$

::: {.panel-tabset}

### R

2\. Оцініть het.-robust standard errors за допомогою опції `coeftest()` з `lmtest` + `vcovHC()` з `sandwich`

```{r, fixest2a r, eval = T}
library(lmtest)
library(sandwich)

coeftest(test_reg, vcov = vcovHC(test_reg, type = "HC0"))[, 1:4] %>% 
  as_tibble() %>% knitr::kable()
```

### Python

2\. Оцініть het.-robust standard errors за допомогою `cov_type = 'HC0'`

```{python, fixest2 p, eval = T}

# Het.-robust standard errors with 'cov_type = 'HC3''
test_reg.get_robustcov_results(cov_type = "HC0").summary().tables[1]
```

:::

```{r, fixest3, include = F}

p_load(fixest)
# Regress log score on ratio and log income
test_reg <- feols(test_score ~ ratio + income, data = test_df)
test_het_out <- summary(test_reg, vcov = 'hetero') %>% capture.output()
test_hom_out <- summary(test_reg, vcov = 'iid') %>% capture.output()
```

# Приклад: WLS

## Приклад: WLS

Ми згадували, що WLS часто неможливий — нам потрібно знати функціональну форму гетероскедастичності — або

**A**\. $\sigma_i^2$

або

**B**\. $h(x_i)$, де $\sigma_i^2 = \sigma^2 h(x_i)$

. . .

Є випадки, коли ми можемо знати $h(x_i)$.

---

## Приклад: WLS {.smaller}

Уявіть, що окремі спостереження в генеральній сукупності мають гомоскедастичні розлади.

Однак замість того, щоб спостерігати за окремими даними, ми спостерігаємо (у даних) середні значення для груп (_наприклад_, міст, округів, шкільних округів).

Якщо ці групи мають різні розміри, тоді наш набір даних буде гетероскедастичним.

**Нагадуємо:** Дисперсія вибіркового середнього залежить від розміру вибірки,
$$ \mathop{\text{Var}} \left( \overline{x} \right) = \dfrac{\sigma_x^2}{n} $$

. . .

**Приклад:** Наші дані шкільного тестування усереднені на рівні школи.

---

## Приклад: WLS {.smaller}

*Приклад:* Наші дані шкільного тестування усереднені на рівні школи.

Якщо окремі учні мають гомоскедастичні порушення, у школах будуть гетероскедастичні порушення, _тобто_,

**Модель індивідуального рівня:** $\quad \text{Score}_i = \beta_0 + \beta_1 \text{Ratio}_i + \beta_2 \text{Income}_i + u_i$

**Модель шкільного рівня:** $\quad \overline{\text{Score}}_s = \beta_0 + \beta_1 \overline{\text{Ratio}}_s + \beta_2 \overline{\text{Income}}_s + \overline{u}_s$

де індекс $s$ позначає окрему школу (так само, як $i$ індексує окрему особу).

$$ \mathop{\text{Var}} \left( \overline{u}_s \right) = \dfrac{\sigma^2}{n_s} $$

---

## Приклад: WLS

Для WLS ми шукаємо функцію $h(x_s)$ таку, що $\mathop{\text{Var}} \left( \overline{u}_s | x_s \right) = \sigma^2 h(x_s )$.

. . .

Ми щойно показали, що $\mathop{\text{Var}} \left( \overline{u}_s |x_s \right) = \dfrac{\sigma^2}{ n_s}$.

. . .

Таким чином, $h(x_s) = 1/n_s$, де $n_s$ – кількість учнів у школі $s$.

. . .

Щоб реалізувати WLS, ми ділимо дані кожного спостереження на $1/\sqrt{h(x_s)}$, тобто нам потрібно помножити дані кожної школи на $\sqrt{n_s}$.

. . .

Змінна `enrollment` у наборі даних `test_df` є нашим $n_s$.

---

## Приклад: WLS

Використання WLS для оцінки $\text{Score}_i = \beta_0 + \beta_1 \text{Ratio}_i + \beta_2 \text{Income}_i + u_i$

**Крок 1:** Помножте кожну змінну на $1/\sqrt{h(x_i)} = \sqrt{\text{Enrollment}_i}$

::: {.panel-tabset}

### R

```{r, wls1 r}
test_df <- mutate(test_df,
  test_score_wls = test_score * sqrt(enrollment),
  ratio_wls      = ratio * sqrt(enrollment),
  income_wls     = income * sqrt(enrollment),
  intercept_wls  = 1 * sqrt(enrollment)
)
```

### Python

```{python, include = F}
import pandas as pd
from statsmodels.datasets import get_rdataset

caschool = get_rdataset("Caschool", "Ecdat")

test_df = caschool.data[["testscr", "str", "avginc", "enrltot"]]
test_df.columns = ["test_score", "ratio", "income", "enrollment"]
```

```{python, wls1 p}
test_df["test_score_wls"] = test_df["test_score"] * (test_df["enrollment"] ** 0.5)
test_df["ratio_wls"] = test_df["ratio"] * (test_df["enrollment"] ** 0.5)
test_df["income_wls"] = test_df["income"] * (test_df["enrollment"] ** 0.5)
test_df["intercept_wls"] = 1 * (test_df["enrollment"] ** 0.5)
```

:::

Зверніть увагу, що ми створюємо перетворене значення `intercept`

---

## Приклад: WLS

Використання WLS для оцінки $\text{Score}_i = \beta_0 + \beta_1 \text{Ratio}_i + \beta_2 \text{Income}_i + u_i$

**Крок 2:** Запустіть нашу регресію WLS (трансформовану).

::: {.panel-tabset}

### R

```{r, wls2 r}
wls_reg <- lm(
  test_score_wls ~ -1 + intercept_wls + ratio_wls + income_wls,
  data = test_df
)
```

### Python

```{python, wls2 p}
wls_reg = smf.ols(
  "test_score_wls ~ -1 + intercept_wls + ratio_wls + income_wls",
  data = test_df
).fit()
```

:::

. . .

*Примітка: * «-1» у нашій регресії повідомляє не додавати `intercept`, оскільки ми додаємо перетворений `intercept` (`intercept_wls`).

---

## Приклад: WLS

[Oцінки WLS і стандартні помилки:]{.hi}

```{r, wls3, echo = F}
# Grab the summary
test_wls_out <- summary(wls_reg) %>% capture.output()
# Print the coefficients
test_wls_out[11:14] %>% paste0("\n") %>% cat()
```

. . .

<br>
Стандартні помилки [МНК]{.hi} і [het.-robust:]{.hi}

```{r, wls4, echo = F}
# Print the coefficients
test_het_out[4:7] %>% paste0("\n") %>% cat()
```

---

## Приклад: WLS
`R`: Альтернатива власному зважуванню: надайте `lm()` деякі `weights`.

```{r, wls5 r, eval = F}
lm(test_score ~ ratio + income, data = test_df, weights = enrollment)
```

`Python`: Альтернатива власному зважуванню: надайте `smf.wls()` деякі `weights`.

```{python, wls5 p, eval = F}
smf.wls("test_score ~ ratio + income", data = test_df, weights = test_df["enrollment"]).fit()
```

---

## Приклад: WLS {.smaller}

У цьому прикладі

- [Гетероскедастично-робасті стандартні помилки]{.hi} не сильно змінили наші стандартні помилки (порівняно з простими стандартними помилками OLS).

- [WLS]{.hi} дещо змінив наші відповіді — коефіцієнти та стандартні помилки.

. . .

Ці приклади підкреслили кілька речей:

1. Використання правильного оцінювача стандартних помилок *має значення*.

2. Статистичні дослідження не завжди пропонують очевидно *правильний* маршрут.

. . .

Щоб побачити \#1, давайте запустимо симуляцію.

---

## Симуляція {.smaller}

```{r, sim params, include = F}
b0 <- 1L
b1 <- 10L
```

Давайте розглянемо просту модель лінійної регресії з гетероскедастичністю.

$$ y_i = \underbrace{\beta_0}_{=`r b0`} + \underbrace{\beta_1}_{=`r b1`} x_i + u_i $$
де $\mathop{\text{Var}} \left( u_i | x_i \right) = \sigma_i^2 = \sigma^2 x_i^2$.

. . .

```{r, sim plot y, echo = F, fig.height = 4}
set.seed(12345)
ggplot(data = tibble(
  x = runif(1e3, 0.5, 1.5),
  y = b0 + b1 * x + rnorm(1e3, 0, sd = x^2)
), aes(x = x, y = y)) +
geom_point(color = "darkslategrey", size = 2.75, alpha = 0.5) +
geom_abline(intercept = b0, slope = b1, color = "orange", size = 1.5, alpha = 0.85) +
labs(x = "x", y = "y") +
theme_axes_math
```

---

## Симуляція {.smaller}

Давайте розглянемо просту модель лінійної регресії з гетероскедастичністю.

$$ y_i = \underbrace{\beta_0}_{=`r b0`} + \underbrace{\beta_1}_{=`r b1`} x_i + u_i $$
де $\mathop{\text{Var}} \left( u_i | x_i \right) = \sigma_i^2 = \sigma^2 x_i^2$.

```{r, sim plot u, echo = F, fig.height = 4}
set.seed(12345)
ggplot(data = tibble(
  x = runif(1e3, 0.5, 1.5),
  u = rnorm(1e3, 0, sd = x^2)
), aes(x = x, y = u)) +
geom_point(color = "darkslategrey", size = 2.75, alpha = 0.5) +
labs(x = "x", y = "u") +
theme_axes_math
```

---

## Симуляція

*Примітка щодо WLS:*

Оскільки $\mathop{\text{Var}} \left( u_i | x_i \right) = \sigma^2 x_i^2$,

$$ \mathop{\text{Var}} \left( u_i | x_i \right) = \sigma^2 h(x_i) \implies h(x_i) = x_i^2 $$

WLS множить кожну змінну на $1/\sqrt{h(x_i)} = 1/x_i$.

---

## Симуляція {.smaller}

У цій симуляції ми хочемо порівняти

1. **Ефективність**
   - OLS
   - WLS з правильними вагами: $h(x_i) = x_i$
   - WLS з неправильними вагами: $h(x_i) = \sqrt{x_i}$

2. Наскільки ефективні наші **стандартні помилки** (через довірчі інтервали) зі
   - Звичайні стандартні помилки OLS
   - Heteroskedasticity-robust standard errors
   - Стандартні помилки WLS

---

## Симуляція {.smaller}

Алгоритм симуляції:

Виконайте 10 000 разів:

1. Згенеруйте вибірку розміром 30 із сукупності

2. Обчисліть/збережіть оцінки OLS та WLS (×2) для β~1~

3. Обчисліть/збережіть стандартні помилки для використання β~1~.
   - Звичайні стандартні помилки OLS
   - Heteroskedasticity-robust standard errors
   - WLS (правильно)
   - WLS (неправильно)

<!-- ---

## Симуляція

**Для однієї ітерації симуляції:**

Код для створення даних...

```{r, sim one iteration 1, eval = F}
# Parameters
b0 <- 1
b1 <- 10
s2 <- 1
# Sample size
n <- 30
# Generate data
sample_df <- tibble(
  x = runif(n, 0.5, 1.5),
  y = b0 + b1 * x + rnorm(n, 0, sd = s2 * x^2)
)
```

---

## Симуляція

**Для однієї ітерації симуляції:**

Код для оцінки наших коефіцієнтів і стандартних помилок...

```{r, sim one iteration 2, eval = F}
# OLS
ols <- feols(y ~ x, data = sample_df)
# WLS: Correct weights
wls_t <- lm(y ~ x, data = sample_df, weights = 1/x^2)
# WLS: Correct weights
wls_f <- lm(y ~ x, data = sample_df, weights = 1/x)
# Coefficients and standard errors
summary(ols, vcov = 'iid')
summary(ols, vcov = 'hetero')
summary(wls_t)
summary(wls_f)
```

Потім збережіть результати.

--- -->

## Симуляція: коефіцієнти

```{r, sim df, include = F, cache = T}
# Parameters
b0 <- 1
b1 <- 10
s2 <- 1
# Sample size
n <- 30
# Number of iterations
n_iter <- 1e3
# Set seed
set.seed(1234)
# The simulation
sim_df <- mclapply(X = 1:n_iter, FUN = function(i, size) {
  # Generate data
  sample_df <- tibble(
    x = runif(size, 0.5, 1.5),
    y = b0 + b1 * x + rnorm(size, 0, sd = s2 * x^2)
  )
  # OLS
  ols <- feols(y ~ x, data = sample_df)
  # WLS: Correct weights
  wls_t <- lm(y ~ x, data = sample_df, weights = 1/x^2)
  # WLS: Correct weights
  wls_f <- lm(y ~ x, data = sample_df, weights = 1/x)
  # Save results
  iter_df <- rbind(
    summary(ols, vcov = 'iid') %>% coeftable() %>% magrittr::extract(2, 1:2),
    summary(ols, vcov = 'hetero') %>% coeftable() %>% magrittr::extract(2, 1:2),
    summary(wls_t) %>% coef() %>% magrittr::extract(2,1:2),
    summary(wls_f) %>% coef() %>% magrittr::extract(2,1:2)
  ) %>%
  as_tibble() %>%
  mutate(
    model = c("OLS Hom.", "OLS Het.", "WLS T", "WLS F"),
    iter = i
  )
  # Return the data
  return(iter_df)
}, size = n) %>% bind_rows()
# Change names
names(sim_df) <- c("coef", "se", "model", "iter")
```

```{r, sim plot efficiency, echo = F, fig.height = 6.25}
ggplot(data = sim_df %>% filter(model != "OLS Hom."), aes(x = coef, color = model, fill = model)) +
geom_vline(xintercept = 10, linetype = "dashed") +
geom_density(alpha = 0.1) +
geom_hline(yintercept = 0) +
labs(x = "Estimated coefficient", y = "Density") +
scale_color_viridis_d("",
  labels = c("OLS", "WLS Incorrect", "WLS Correct"),
  end = 0.9, option = "C"
) +
scale_fill_viridis_d("",
  labels = c("OLS", "WLS Incorrect", "WLS Correct"),
  end = 0.9, option = "C"
) +
theme_pander(base_size = 22, base_family = "Fira Sans") +
theme(
  legend.position = c(.85,.9),
  # legend.background = element_blank(),
  legend.key.size = unit(1, "cm")
)
```

---

## Симуляція: висновок

```{r, sim plot t stat, echo = F, fig.height = 6.25}
ggplot(data = sim_df, aes(x = (coef-10)/se, color = model, fill = model)) +
geom_vline(xintercept = qt(c(0.025, 0.975), df = 28), linetype = "dashed") +
geom_density(alpha = 0.1) +
geom_hline(yintercept = 0) +
labs(x = "t statistic testing the true value", y = "Density") +
scale_color_viridis_d("",
  labels = c("OLS + Het.-robust", "Plain OLS", "WLS Incorrect", "WLS Correct"),
  end = 0.9, option = "C"
) +
scale_fill_viridis_d("",
labels = c("OLS + Het.-robust", "Plain OLS", "WLS Incorrect", "WLS Correct"),
  end = 0.9, option = "C"
) +
theme_pander(base_size = 22, base_family = "Fira Sans") +
theme(
  legend.position = c(.85,.9),
  # legend.background = element_blank(),
  legend.key.size = unit(1, "cm")
)
```

---

## Симуляція: результат

Узагальнення результатів нашого моделювання (10 000 ітерацій)

:::: {.columns}

::: {.column width="40%"}
**Оцінка**: Summary $\hat{\beta}_1$
```{r, sim table coef, eval = T, echo = F}
sim_df %>%
  filter(model != "OLS Hom.") %>%
  mutate(Estimator = recode(model,
    "OLS Het." = "OLS",
    "WLS F" = "WLS Incorrect",
    "WLS T" = "WLS Correct"
  )) %>%
  group_by(Estimator) %>%
  summarize(Mean = mean(coef) %>% round(3), "S.D." = sd(coef) %>% round(3)) %>%
  kable() %>%
  kable_styling(full_width = T)
```
:::

::: {.column width="60%"}
**Висновок: ** % випадків ми відхиляємо $\beta_1$
```{r, sim table se, eval = T, echo = F}
sim_df %>%
  mutate(Estimators = recode(model,
    "OLS Hom." = "OLS + Homosk.",
    "OLS Het." = "OLS + Het.-robust",
    "WLS F" = "WLS Incorrect",
    "WLS T" = "WLS Correct"
  )) %>%
  group_by(Estimators) %>%
  summarize(`% Reject` = mean(abs(coef-10)/se > qt(0.975, 28)) %>% multiply_by(100) %>% round(1)) %>%
  kable() %>%
  kable_styling(full_width = T)
```
:::

::::

---

## Припущення

Згадайте наше старе припущення, яке призвело до обговорення гетероскедастичності:

> 5\. Збурення мають [**постійну дисперсія**]{.slate} $\sigma^2$ <br>та [нульову коваріацію]{.hi}, _тобто_,
  - $\color{#314f4f}{\mathop{\boldsymbol{E}}\left[ u_i^2 \middle| X_i \right] = \mathop{\text{Var}} \left( u_i \middle| X_i \right) = \sigma^2 \implies \mathop{\text{Var}} \left( u_i \right) = \sigma^2}$
  - $\color{#e64173}{\mathop{\text{Cov}} \left( u_i, \, u_j \middle| X_i,\,X_j \right) = \mathop{\boldsymbol{E}}\left[ u_i u_j \middle| X_i,\,X_j \right] = 0}$ for $i\neq j$

. . .

[**Порушення постійної дисперсії**]{.slate} $=$ [**heteroskedasticity**]{.slate}

Також можливо (імовірно), що [збурення корельовані]{.pink}.

Ігнорування цієї кореляції ще більш проблематично для висновків.

---

## Проблема {.smaller}

У багатьох випадках [збурення спостережень]{.pink} $\left(u_i,\,u_j\right)$ [можуть бути корелювані]{.pink}.

*Запам'ятай*

- [збурення]{.pink} представляє невключені змінні, які впливають на $y$.
- Деякі [спостереження]{.pink} у вибірці можуть *корельовані* з іншими спостереженнями.

Якщо ці взаємозв’язки на рівні спостереження поширюються на змінні в збуренні, тоді збурення можуть корелювати.

$\implies \mathop{\text{Cov}} \left( u_i, \, u_j \middle| X_i,\,X_j \right) \color{#e64173}{\neq} 0$.

. . .

Ігнорування цієї кореляції може спричинити **великі проблеми** у вашому висновку.

---

## Інтуїція

Чому ігнорування такої кореляції призводить до проблем?

**Помилкова точність**: ми можемо стати «надто впевненими» у своїх знаннях.

**Екстремальний приклад**: якщо дублювати свій набір даних (складати його поверх самого себе), звичайні стандартні помилки OLS зменшуватимуться кожного разу, коли ви дублюватимете набір даних.

---

```{r, dup-data, echo = F, cache = TRUE}
# And an id
test_df %<>% mutate(id = 1:nrow(test_df))
# Stack the dataset a bunch of times
dup_df = lapply(X = 1:20, FUN = function(x) mutate(test_df, dup = x)) %>% bind_rows()
# Now run a regression
dup_est = lapply(
  X = 1:20,
  FUN = function(x) feols(test_score ~ ratio + income, data = dup_df %>% filter(dup <= x))
)
# Grab standard errors
dup_se = lapply(
  X = seq_along(dup_est),
  FUN = function(x) {
    data.frame(
      dups = x - 1,
      se_iid = dup_est[[x]] %>% summary(vcov = 'iid') %>% coeftable() %>% magrittr::extract(2,2),
      se_cl = dup_est[[x]] %>% summary(cluster = 'id') %>% coeftable() %>% magrittr::extract(2,2)
    )
  }
) %>% bind_rows()
```

Вплив дублювання наших даних на [стандартні помилки МНК]{.pink} параметра `ratio`.

```{r, dup-plot1, echo = FALSE}
ggplot(
  data = dup_se,
  aes(x = dups, y = se_iid)
) +
geom_hline(yintercept = 0) +
geom_line(color = red_pink, size = 0.5) +
geom_point(color = red_pink, size = 3.5) +
labs(x = "Number of times dataset is duplicated", y = "Standard error") +
theme_pander(base_size = 22, base_family = "Fira Sans Condensed")
```

---

Виправляємо наші [стандартні помилки]{.orange} (кореляція спостережень).

```{r, dup-plot2, echo = FALSE}
ggplot(
  data = dup_se,
  aes(x = dups)
) +
geom_hline(yintercept = 0) +
geom_line(aes(y = se_iid), color = red_pink, size = 0.5, alpha = 0.25) +
geom_point(aes(y = se_iid), color = red_pink, size = 3.5, alpha = 0.25) +
geom_line(aes(y = se_cl), color = orange, size = 0.5) +
geom_point(aes(y = se_cl), color = orange, size = 3.5) +
labs(x = "Number of times dataset is duplicated", y = "Standard error") +
theme_pander(base_size = 22, base_family = "Fira Sans Condensed")
```

---

## Приклади

«Реальні» приклади, коли збурення можуть корелювати:

- Учні в класі (діляться вчителем, навчальним планом тощо)
- Підприємства в місті
- Послідовні дні у вибірці (поділіться подіями, погодою тощо)

---

## Рішення

Подібно до того, як ми обчислюємо *heteroskedasticity*-robust standard errors, ми також можемо розрахувати стандартні помилки, стійкі до корельованих збурень.

Це називається [*cluster-robust standard errors*]{.pink} (або просто *clustered*).

---

## Кращий висновок

1. Ви повинні за замовчуванням вважати, що ваші дані гетероскедастичні

2. Подумайте про те, як ваші пояснювальні змінні та/або порушення корелюють між спостереженнями.

# Додаткові матеріали



# Дякую за увагу! {background-color="#eef3f8" .unlisted}

<br>
<br>

{{< iconify ic baseline-telegram >}} [@araprof](https://t.me/araprof)

{{< iconify mdi youtube >}} [@datamirosh](https://www.youtube.com/@datamirosh)

{{< iconify ion home >}} [aranaur.rbind.io](https://aranaur.rbind.io)

{{< iconify mdi github >}} [aranaur](https://github.com/Aranaur)

{{< iconify mdi envelope >}} <ihor.miroshnychenko@knu.ua>

{{< iconify mdi linkedin >}} [ihormiroshnychenko](https://www.linkedin.com/in/ihormiroshnychenko/)
